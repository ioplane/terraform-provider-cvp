...
| **Provider-defined functions** | 1.8 | Pure helpers: `provider::cvp::studio_path(...)` to build/validate a Studio input path (bracket notation), `provider::cvp::merge_inputs(...)` for `inputs_json` composition. Offline + pure (HashiCorp design principle 6/7). | **P2** |
