// Copyright (c) 2020 Arista Networks, Inc.  All rights reserved.
// Use of this source code is governed by the Apache License 2.0
// that can be found in the COPYING file.

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        (unknown)
// source: arista/changecontrol.v1/changecontrol.proto

package changecontrolv1

import (
	fmp "github.com/ioplane/terraform-provider-cvp/internal/pb/fmp"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	wrapperspb "google.golang.org/protobuf/types/known/wrapperspb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// StageStatus defines the possible execution statuses of a stage.
type StageStatus int32

const (
	// STAGE_STATUS_UNSPECIFIED means unknown stage status.
	StageStatus_STAGE_STATUS_UNSPECIFIED StageStatus = 0
	// STAGE_STATUS_RUNNING means the stage has begun execution.
	StageStatus_STAGE_STATUS_RUNNING StageStatus = 1
	// STAGE_STATUS_COMPLETED means the stage has ceased execution.
	// Success/failure of a stage cannot be inferred from this status
	// alone but rather this status plus the stage error. That is, no
	// error implies success and some error implies failure.
	StageStatus_STAGE_STATUS_COMPLETED StageStatus = 2
	// STAGE_STATUS_NOT_STARTED means the stage has not been started.
	StageStatus_STAGE_STATUS_NOT_STARTED StageStatus = 3
)

// Enum value maps for StageStatus.
var (
	StageStatus_name = map[int32]string{
		0: "STAGE_STATUS_UNSPECIFIED",
		1: "STAGE_STATUS_RUNNING",
		2: "STAGE_STATUS_COMPLETED",
		3: "STAGE_STATUS_NOT_STARTED",
	}
	StageStatus_value = map[string]int32{
		"STAGE_STATUS_UNSPECIFIED": 0,
		"STAGE_STATUS_RUNNING":     1,
		"STAGE_STATUS_COMPLETED":   2,
		"STAGE_STATUS_NOT_STARTED": 3,
	}
)

func (x StageStatus) Enum() *StageStatus {
	p := new(StageStatus)
	*p = x
	return p
}

func (x StageStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (StageStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_arista_changecontrol_v1_changecontrol_proto_enumTypes[0].Descriptor()
}

func (StageStatus) Type() protoreflect.EnumType {
	return &file_arista_changecontrol_v1_changecontrol_proto_enumTypes[0]
}

func (x StageStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use StageStatus.Descriptor instead.
func (StageStatus) EnumDescriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{0}
}

// StepStatus defines the possible execution statuses of a step within an action.
type StepStatus int32

const (
	// STEP_STATUS_UNSPECIFIED represents an unspecified state for a step within
	// an action.
	StepStatus_STEP_STATUS_UNSPECIFIED StepStatus = 0
	// STEP_STATUS_NOT_STARTED represents that the step has not started execution.
	StepStatus_STEP_STATUS_NOT_STARTED StepStatus = 1
	// STEP_STATUS_RUNNING represents that the step is under execution.
	StepStatus_STEP_STATUS_RUNNING StepStatus = 2
	// STEP_STATUS_FINISHED represents that the step has finished execution.
	StepStatus_STEP_STATUS_FINISHED StepStatus = 3
	// STEP_STATUS_SKIPPED represents that the step has been skipped from execution.
	StepStatus_STEP_STATUS_SKIPPED StepStatus = 4
	// STEP_STATUS_FAILED represents that the step has failed its execution.
	StepStatus_STEP_STATUS_FAILED StepStatus = 5
)

// Enum value maps for StepStatus.
var (
	StepStatus_name = map[int32]string{
		0: "STEP_STATUS_UNSPECIFIED",
		1: "STEP_STATUS_NOT_STARTED",
		2: "STEP_STATUS_RUNNING",
		3: "STEP_STATUS_FINISHED",
		4: "STEP_STATUS_SKIPPED",
		5: "STEP_STATUS_FAILED",
	}
	StepStatus_value = map[string]int32{
		"STEP_STATUS_UNSPECIFIED": 0,
		"STEP_STATUS_NOT_STARTED": 1,
		"STEP_STATUS_RUNNING":     2,
		"STEP_STATUS_FINISHED":    3,
		"STEP_STATUS_SKIPPED":     4,
		"STEP_STATUS_FAILED":      5,
	}
)

func (x StepStatus) Enum() *StepStatus {
	p := new(StepStatus)
	*p = x
	return p
}

func (x StepStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (StepStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_arista_changecontrol_v1_changecontrol_proto_enumTypes[1].Descriptor()
}

func (StepStatus) Type() protoreflect.EnumType {
	return &file_arista_changecontrol_v1_changecontrol_proto_enumTypes[1]
}

func (x StepStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use StepStatus.Descriptor instead.
func (StepStatus) EnumDescriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{1}
}

// ChangeControlStatus defines the possible execution statuses of
// a Change Control.
type ChangeControlStatus int32

const (
	// CHANGE_CONTROL_STATUS_UNSPECIFIED means the Change Control status is unknown.
	ChangeControlStatus_CHANGE_CONTROL_STATUS_UNSPECIFIED ChangeControlStatus = 0
	// CHANGE_CONTROL_STATUS_RUNNING means the Change Control has begun
	// execution.
	ChangeControlStatus_CHANGE_CONTROL_STATUS_RUNNING ChangeControlStatus = 1
	// CHANGE_CONTROL_STATUS_COMPLETED means the Change Control has ceased
	// execution. Success/failure of a Change Control cannot be inferred
	// from this status alone but rather this status plus the Change Control
	// error. That is, no error implies success and some error implies failure.
	ChangeControlStatus_CHANGE_CONTROL_STATUS_COMPLETED ChangeControlStatus = 2
	// CHANGE_CONTROL_STATUS_SCHEDULED means the Change Control has been
	// scheduled for execution at some time. Any failure that occurs during
	// this process will cause a transition back to the unspecified status,
	// a reset of the schedule flag by the system, and an error on the change
	// control reporting the details of the failure.
	ChangeControlStatus_CHANGE_CONTROL_STATUS_SCHEDULED ChangeControlStatus = 3
	// CHANGE_CONTROL_STATUS_NOT_STARTED means the Change Control has not been
	// started. This would include approved and not approved Change Controls.
	ChangeControlStatus_CHANGE_CONTROL_STATUS_NOT_STARTED ChangeControlStatus = 4
)

// Enum value maps for ChangeControlStatus.
var (
	ChangeControlStatus_name = map[int32]string{
		0: "CHANGE_CONTROL_STATUS_UNSPECIFIED",
		1: "CHANGE_CONTROL_STATUS_RUNNING",
		2: "CHANGE_CONTROL_STATUS_COMPLETED",
		3: "CHANGE_CONTROL_STATUS_SCHEDULED",
		4: "CHANGE_CONTROL_STATUS_NOT_STARTED",
	}
	ChangeControlStatus_value = map[string]int32{
		"CHANGE_CONTROL_STATUS_UNSPECIFIED": 0,
		"CHANGE_CONTROL_STATUS_RUNNING":     1,
		"CHANGE_CONTROL_STATUS_COMPLETED":   2,
		"CHANGE_CONTROL_STATUS_SCHEDULED":   3,
		"CHANGE_CONTROL_STATUS_NOT_STARTED": 4,
	}
)

func (x ChangeControlStatus) Enum() *ChangeControlStatus {
	p := new(ChangeControlStatus)
	*p = x
	return p
}

func (x ChangeControlStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ChangeControlStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_arista_changecontrol_v1_changecontrol_proto_enumTypes[2].Descriptor()
}

func (ChangeControlStatus) Type() protoreflect.EnumType {
	return &file_arista_changecontrol_v1_changecontrol_proto_enumTypes[2]
}

func (x ChangeControlStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ChangeControlStatus.Descriptor instead.
func (ChangeControlStatus) EnumDescriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{2}
}

// CompletionReason describes the reason behind the terminal state of an executed Change Control.
type CompletionReason int32

const (
	// COMPLETION_REASON_UNSPECIFIED means the completion reason is unknown.
	CompletionReason_COMPLETION_REASON_UNSPECIFIED CompletionReason = 0
	// COMPLETION_REASON_SUCCESS means the Change Control completed successfully.
	CompletionReason_COMPLETION_REASON_SUCCESS CompletionReason = 1
	// COMPLETION_REASON_FAILED means the Change Control completed due to failure.
	CompletionReason_COMPLETION_REASON_FAILED CompletionReason = 2
	// COMPLETION_REASON_STOPPED means the Change Control was stopped by a user.
	CompletionReason_COMPLETION_REASON_STOPPED CompletionReason = 3
)

// Enum value maps for CompletionReason.
var (
	CompletionReason_name = map[int32]string{
		0: "COMPLETION_REASON_UNSPECIFIED",
		1: "COMPLETION_REASON_SUCCESS",
		2: "COMPLETION_REASON_FAILED",
		3: "COMPLETION_REASON_STOPPED",
	}
	CompletionReason_value = map[string]int32{
		"COMPLETION_REASON_UNSPECIFIED": 0,
		"COMPLETION_REASON_SUCCESS":     1,
		"COMPLETION_REASON_FAILED":      2,
		"COMPLETION_REASON_STOPPED":     3,
	}
)

func (x CompletionReason) Enum() *CompletionReason {
	p := new(CompletionReason)
	*p = x
	return p
}

func (x CompletionReason) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (CompletionReason) Descriptor() protoreflect.EnumDescriptor {
	return file_arista_changecontrol_v1_changecontrol_proto_enumTypes[3].Descriptor()
}

func (CompletionReason) Type() protoreflect.EnumType {
	return &file_arista_changecontrol_v1_changecontrol_proto_enumTypes[3]
}

func (x CompletionReason) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use CompletionReason.Descriptor instead.
func (CompletionReason) EnumDescriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{3}
}

// OverrideReason provides a reason for overriding the validation.
type OverrideReason int32

const (
	// OVERRIDE_REASON_UNSPECIFIED indicates that the reason for overriding the validation
	// is unknown.
	OverrideReason_OVERRIDE_REASON_UNSPECIFIED OverrideReason = 0
	// OVERRIDE_REASON_USER_FORCED indicates that the user has overridden the validation and forced
	// the approval of the Change Control.
	OverrideReason_OVERRIDE_REASON_USER_FORCED OverrideReason = 1
)

// Enum value maps for OverrideReason.
var (
	OverrideReason_name = map[int32]string{
		0: "OVERRIDE_REASON_UNSPECIFIED",
		1: "OVERRIDE_REASON_USER_FORCED",
	}
	OverrideReason_value = map[string]int32{
		"OVERRIDE_REASON_UNSPECIFIED": 0,
		"OVERRIDE_REASON_USER_FORCED": 1,
	}
)

func (x OverrideReason) Enum() *OverrideReason {
	p := new(OverrideReason)
	*p = x
	return p
}

func (x OverrideReason) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (OverrideReason) Descriptor() protoreflect.EnumDescriptor {
	return file_arista_changecontrol_v1_changecontrol_proto_enumTypes[4].Descriptor()
}

func (OverrideReason) Type() protoreflect.EnumType {
	return &file_arista_changecontrol_v1_changecontrol_proto_enumTypes[4]
}

func (x OverrideReason) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use OverrideReason.Descriptor instead.
func (OverrideReason) EnumDescriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{4}
}

// ChangeControlValidationStatus is used to indicate the status of the Change Control validation.
type ChangeControlValidationStatus int32

const (
	// CHANGE_CONTROL_VALIDATION_STATUS_UNSPECIFIED indicates that the validation status
	// is unknown.
	ChangeControlValidationStatus_CHANGE_CONTROL_VALIDATION_STATUS_UNSPECIFIED ChangeControlValidationStatus = 0
	// CHANGE_CONTROL_VALIDATION_STATUS_SUCCESS indicates that the validation has succeeded.
	ChangeControlValidationStatus_CHANGE_CONTROL_VALIDATION_STATUS_SUCCESS ChangeControlValidationStatus = 1
	// CHANGE_CONTROL_VALIDATION_STATUS_OBSOLETED indicates that the validation has failed, and the
	// Change Control has been obsoleted. This is due to the execution of a newer Change Control.
	ChangeControlValidationStatus_CHANGE_CONTROL_VALIDATION_STATUS_OBSOLETED ChangeControlValidationStatus = 2
	// CHANGE_CONTROL_VALIDATION_STATUS_CONCURRENT_EXECUTION indicates that validation failed
	// because execution would conflict with another Change Control that is currently running
	// on one or more target devices. This Change Control is blocked until the conflicting
	// execution completes.
	ChangeControlValidationStatus_CHANGE_CONTROL_VALIDATION_STATUS_CONCURRENT_EXECUTION ChangeControlValidationStatus = 3
)

// Enum value maps for ChangeControlValidationStatus.
var (
	ChangeControlValidationStatus_name = map[int32]string{
		0: "CHANGE_CONTROL_VALIDATION_STATUS_UNSPECIFIED",
		1: "CHANGE_CONTROL_VALIDATION_STATUS_SUCCESS",
		2: "CHANGE_CONTROL_VALIDATION_STATUS_OBSOLETED",
		3: "CHANGE_CONTROL_VALIDATION_STATUS_CONCURRENT_EXECUTION",
	}
	ChangeControlValidationStatus_value = map[string]int32{
		"CHANGE_CONTROL_VALIDATION_STATUS_UNSPECIFIED":          0,
		"CHANGE_CONTROL_VALIDATION_STATUS_SUCCESS":              1,
		"CHANGE_CONTROL_VALIDATION_STATUS_OBSOLETED":            2,
		"CHANGE_CONTROL_VALIDATION_STATUS_CONCURRENT_EXECUTION": 3,
	}
)

func (x ChangeControlValidationStatus) Enum() *ChangeControlValidationStatus {
	p := new(ChangeControlValidationStatus)
	*p = x
	return p
}

func (x ChangeControlValidationStatus) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (ChangeControlValidationStatus) Descriptor() protoreflect.EnumDescriptor {
	return file_arista_changecontrol_v1_changecontrol_proto_enumTypes[5].Descriptor()
}

func (ChangeControlValidationStatus) Type() protoreflect.EnumType {
	return &file_arista_changecontrol_v1_changecontrol_proto_enumTypes[5]
}

func (x ChangeControlValidationStatus) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use ChangeControlValidationStatus.Descriptor instead.
func (ChangeControlValidationStatus) EnumDescriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{5}
}

// DeviceValidationFailure is used to indicate the type of the validation failure.
type DeviceValidationFailure int32

const (
	// DEVICE_VALIDATION_FAILURE_UNSPECIFIED indicates that the reason for the validation failure
	// is unknown.
	DeviceValidationFailure_DEVICE_VALIDATION_FAILURE_UNSPECIFIED DeviceValidationFailure = 0
	// DEVICE_VALIDATION_FAILURE_CONFIG_OUT_OF_ORDER indicates that the designed config is out of
	// order due to the execution of a newer Change Control.
	DeviceValidationFailure_DEVICE_VALIDATION_FAILURE_CONFIG_OUT_OF_ORDER DeviceValidationFailure = 1
	// DEVICE_VALIDATION_FAILURE_IMAGE_OUT_OF_ORDER indicates that the designed image is out of
	// order due to the execution of a newer Change Control.
	DeviceValidationFailure_DEVICE_VALIDATION_FAILURE_IMAGE_OUT_OF_ORDER DeviceValidationFailure = 2
	// DEVICE_VALIDATION_FAILURE_CONCURRENT_EXECUTION indicates that the device is currently
	// executing another Change Control.
	DeviceValidationFailure_DEVICE_VALIDATION_FAILURE_CONCURRENT_EXECUTION DeviceValidationFailure = 3
)

// Enum value maps for DeviceValidationFailure.
var (
	DeviceValidationFailure_name = map[int32]string{
		0: "DEVICE_VALIDATION_FAILURE_UNSPECIFIED",
		1: "DEVICE_VALIDATION_FAILURE_CONFIG_OUT_OF_ORDER",
		2: "DEVICE_VALIDATION_FAILURE_IMAGE_OUT_OF_ORDER",
		3: "DEVICE_VALIDATION_FAILURE_CONCURRENT_EXECUTION",
	}
	DeviceValidationFailure_value = map[string]int32{
		"DEVICE_VALIDATION_FAILURE_UNSPECIFIED":          0,
		"DEVICE_VALIDATION_FAILURE_CONFIG_OUT_OF_ORDER":  1,
		"DEVICE_VALIDATION_FAILURE_IMAGE_OUT_OF_ORDER":   2,
		"DEVICE_VALIDATION_FAILURE_CONCURRENT_EXECUTION": 3,
	}
)

func (x DeviceValidationFailure) Enum() *DeviceValidationFailure {
	p := new(DeviceValidationFailure)
	*p = x
	return p
}

func (x DeviceValidationFailure) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (DeviceValidationFailure) Descriptor() protoreflect.EnumDescriptor {
	return file_arista_changecontrol_v1_changecontrol_proto_enumTypes[6].Descriptor()
}

func (DeviceValidationFailure) Type() protoreflect.EnumType {
	return &file_arista_changecontrol_v1_changecontrol_proto_enumTypes[6]
}

func (x DeviceValidationFailure) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use DeviceValidationFailure.Descriptor instead.
func (DeviceValidationFailure) EnumDescriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{6}
}

// RepeatedRepeatedString wraps a repeated `fmp.RepeatedString`
// to define a string matrix which is used to represent stage
// rows (see `StageConfig`).
type RepeatedRepeatedString struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// values is a list of `fmp.RepeatedString`.
	Values        []*fmp.RepeatedString `protobuf:"bytes,1,rep,name=values,proto3" json:"values,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RepeatedRepeatedString) Reset() {
	*x = RepeatedRepeatedString{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RepeatedRepeatedString) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RepeatedRepeatedString) ProtoMessage() {}

func (x *RepeatedRepeatedString) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RepeatedRepeatedString.ProtoReflect.Descriptor instead.
func (*RepeatedRepeatedString) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{0}
}

func (x *RepeatedRepeatedString) GetValues() []*fmp.RepeatedString {
	if x != nil {
		return x.Values
	}
	return nil
}

// ChangeControlKey uniquely identifies a Change Control.
type ChangeControlKey struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// id is the ID of the Change Control.
	Id            *wrapperspb.StringValue `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeControlKey) Reset() {
	*x = ChangeControlKey{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeControlKey) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeControlKey) ProtoMessage() {}

func (x *ChangeControlKey) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeControlKey.ProtoReflect.Descriptor instead.
func (*ChangeControlKey) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{1}
}

func (x *ChangeControlKey) GetId() *wrapperspb.StringValue {
	if x != nil {
		return x.Id
	}
	return nil
}

// Action is an action to perform during the execution of
// a stage of a Change Control. Available actions can be
// fetched using the "action" services.
type Action struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// name is the name of the action.
	Name *wrapperspb.StringValue `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// timeout is the maximum duration in seconds that
	// the action can execute before timing out. If this
	// is not set, then this is interpreted to mean there
	// is no timeout.
	Timeout *wrapperspb.UInt32Value `protobuf:"bytes,2,opt,name=timeout,proto3" json:"timeout,omitempty"`
	// args are the arguments of the action.
	Args          *fmp.MapStringString `protobuf:"bytes,3,opt,name=args,proto3" json:"args,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Action) Reset() {
	*x = Action{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Action) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Action) ProtoMessage() {}

func (x *Action) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Action.ProtoReflect.Descriptor instead.
func (*Action) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{2}
}

func (x *Action) GetName() *wrapperspb.StringValue {
	if x != nil {
		return x.Name
	}
	return nil
}

func (x *Action) GetTimeout() *wrapperspb.UInt32Value {
	if x != nil {
		return x.Timeout
	}
	return nil
}

func (x *Action) GetArgs() *fmp.MapStringString {
	if x != nil {
		return x.Args
	}
	return nil
}

// StageConfig holds a configuration for a stage in a change
// control. Each stage generally defines either an action
// or a series of sub-stages.
type StageConfig struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// name is the name of the stage.
	Name *wrapperspb.StringValue `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// action is the action to perform on stage execution.
	Action *Action `protobuf:"bytes,2,opt,name=action,proto3" json:"action,omitempty"`
	// rows is a series of rows of parallel stages referenced
	// by ID. Each row is run one after the other and the
	// stages within each row are run in parallel.
	//
	// For example:
	//
	// ```
	// [["1a", "1b"], ["2"]]
	// ```
	//
	// This configures stage 1a and stage 1b to run at the
	// same time, and then stage 2 once both of them have
	// completed.
	Rows          *RepeatedRepeatedString `protobuf:"bytes,3,opt,name=rows,proto3" json:"rows,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StageConfig) Reset() {
	*x = StageConfig{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StageConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StageConfig) ProtoMessage() {}

func (x *StageConfig) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StageConfig.ProtoReflect.Descriptor instead.
func (*StageConfig) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{3}
}

func (x *StageConfig) GetName() *wrapperspb.StringValue {
	if x != nil {
		return x.Name
	}
	return nil
}

func (x *StageConfig) GetAction() *Action {
	if x != nil {
		return x.Action
	}
	return nil
}

func (x *StageConfig) GetRows() *RepeatedRepeatedString {
	if x != nil {
		return x.Rows
	}
	return nil
}

// StageConfigMap wraps a map from stage ID to `StageConfig`.
// This defines the configuration and order of execution
// for the stages in a Change Control.
//
// For example:
//
// ```
//
//	{
//	    "root": { name: "root", rows: [["1-2"], ["3"]] },
//	    "1-2":  { name: "stages 1-2", rows: [["1"], ["2"]] },
//	    "1":    { name: "stage 1", rows: [["1a", "1b"]] },
//	    "1a":   { name: "stage 1a", action: { name: "task", args: { "TaskID": "101" } },
//	    "1b":   { name: "stage 1b", action: { name: "task", args: { "TaskID": "102" } },
//	    "2":    { name: "stage 2", action: { name: "task", args: { "TaskID": "103" } },
//	    "3":    { name: "stage 3", action: { name: "task", args: { "TaskID": "104" } }
//	}
//
// ```
//
// Assuming the root stage ID of the enclosing Change Control
// is "root", this would mean to do the following in sequence:
//
// ```
// root
// |- stages 1-2
// |  |- stage 1
// |  |  |- stage 1a, stage 1b (parallel)
// |  |- stage 2
// |- stage 3
// ```
//
// That is, execute tasks `101` and `102` in parallel, then task `103`, then task `104`.
type StageConfigMap struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// values is a map from stage ID to `StageConfig`.
	Values        map[string]*StageConfig `protobuf:"bytes,1,rep,name=values,proto3" json:"values,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StageConfigMap) Reset() {
	*x = StageConfigMap{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StageConfigMap) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StageConfigMap) ProtoMessage() {}

func (x *StageConfigMap) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StageConfigMap.ProtoReflect.Descriptor instead.
func (*StageConfigMap) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{4}
}

func (x *StageConfigMap) GetValues() map[string]*StageConfig {
	if x != nil {
		return x.Values
	}
	return nil
}

// ChangeConfig holds a configuration for the change of a change
// control which is essentially a named configuration of stages.
type ChangeConfig struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// name is the name of the change.
	Name *wrapperspb.StringValue `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// root_stage_id is the ID of the root stage or the stage that
	// should execute first.
	RootStageId *wrapperspb.StringValue `protobuf:"bytes,2,opt,name=root_stage_id,json=rootStageId,proto3" json:"root_stage_id,omitempty"`
	// stages holds a configuration of stages. See `StageConfigMap`
	// description for more information.
	Stages *StageConfigMap `protobuf:"bytes,3,opt,name=stages,proto3" json:"stages,omitempty"`
	// notes are any notes associated with the change.
	Notes         *wrapperspb.StringValue `protobuf:"bytes,4,opt,name=notes,proto3" json:"notes,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeConfig) Reset() {
	*x = ChangeConfig{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeConfig) ProtoMessage() {}

func (x *ChangeConfig) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeConfig.ProtoReflect.Descriptor instead.
func (*ChangeConfig) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{5}
}

func (x *ChangeConfig) GetName() *wrapperspb.StringValue {
	if x != nil {
		return x.Name
	}
	return nil
}

func (x *ChangeConfig) GetRootStageId() *wrapperspb.StringValue {
	if x != nil {
		return x.RootStageId
	}
	return nil
}

func (x *ChangeConfig) GetStages() *StageConfigMap {
	if x != nil {
		return x.Stages
	}
	return nil
}

func (x *ChangeConfig) GetNotes() *wrapperspb.StringValue {
	if x != nil {
		return x.Notes
	}
	return nil
}

// FlagConfig is used to set a flag on a Change Control that takes
// a boolean value (e.g. start/stop, approve/unapprove).
type FlagConfig struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// value is the value of the flag (`true` or `false`).
	Value *wrapperspb.BoolValue `protobuf:"bytes,1,opt,name=value,proto3" json:"value,omitempty"`
	// notes are any notes associated with the flag value.
	Notes         *wrapperspb.StringValue `protobuf:"bytes,2,opt,name=notes,proto3" json:"notes,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FlagConfig) Reset() {
	*x = FlagConfig{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FlagConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FlagConfig) ProtoMessage() {}

func (x *FlagConfig) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FlagConfig.ProtoReflect.Descriptor instead.
func (*FlagConfig) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{6}
}

func (x *FlagConfig) GetValue() *wrapperspb.BoolValue {
	if x != nil {
		return x.Value
	}
	return nil
}

func (x *FlagConfig) GetNotes() *wrapperspb.StringValue {
	if x != nil {
		return x.Notes
	}
	return nil
}

// TimestampFlagConfig is used to set a flag on a Change Control
// that takes a timestamp value (e.g. schedule/unschedule).
type TimestampFlagConfig struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// value is the value of the flag (some timestamp).
	Value *timestamppb.Timestamp `protobuf:"bytes,1,opt,name=value,proto3" json:"value,omitempty"`
	// notes are the notes associated with the flag value.
	Notes         *wrapperspb.StringValue `protobuf:"bytes,2,opt,name=notes,proto3" json:"notes,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TimestampFlagConfig) Reset() {
	*x = TimestampFlagConfig{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TimestampFlagConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TimestampFlagConfig) ProtoMessage() {}

func (x *TimestampFlagConfig) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TimestampFlagConfig.ProtoReflect.Descriptor instead.
func (*TimestampFlagConfig) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{7}
}

func (x *TimestampFlagConfig) GetValue() *timestamppb.Timestamp {
	if x != nil {
		return x.Value
	}
	return nil
}

func (x *TimestampFlagConfig) GetNotes() *wrapperspb.StringValue {
	if x != nil {
		return x.Notes
	}
	return nil
}

// ChangeControlConfig holds the configuration of a Change Control.
type ChangeControlConfig struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// key uniquely identifies the Change Control.
	Key *ChangeControlKey `protobuf:"bytes,1,opt,name=key,proto3" json:"key,omitempty"`
	// change is the change subject to execution.
	Change *ChangeConfig `protobuf:"bytes,2,opt,name=change,proto3" json:"change,omitempty"`
	// start is the flag to start (`start.value` set to `true`)
	// or stop (`start.value` set to `false`) execution of the
	// Change Control.
	Start *FlagConfig `protobuf:"bytes,3,opt,name=start,proto3" json:"start,omitempty"`
	// schedule is the flag to schedule (`schedule.value` set to
	// some timestamp) or unschedule (`schedule.value` set to
	// `nil`) the Change Control for execution.
	Schedule      *TimestampFlagConfig `protobuf:"bytes,4,opt,name=schedule,proto3" json:"schedule,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeControlConfig) Reset() {
	*x = ChangeControlConfig{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeControlConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeControlConfig) ProtoMessage() {}

func (x *ChangeControlConfig) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeControlConfig.ProtoReflect.Descriptor instead.
func (*ChangeControlConfig) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{8}
}

func (x *ChangeControlConfig) GetKey() *ChangeControlKey {
	if x != nil {
		return x.Key
	}
	return nil
}

func (x *ChangeControlConfig) GetChange() *ChangeConfig {
	if x != nil {
		return x.Change
	}
	return nil
}

func (x *ChangeControlConfig) GetStart() *FlagConfig {
	if x != nil {
		return x.Start
	}
	return nil
}

func (x *ChangeControlConfig) GetSchedule() *TimestampFlagConfig {
	if x != nil {
		return x.Schedule
	}
	return nil
}

// StepInfo contains the status details of an action step.
type StepInfo struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// name represents the name of the step.
	Name *wrapperspb.StringValue `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// status represents the present execution status of the step.
	Status StepStatus `protobuf:"varint,2,opt,name=status,proto3,enum=arista.changecontrol.v1.StepStatus" json:"status,omitempty"`
	// error captures any error that occurred.
	Error *wrapperspb.StringValue `protobuf:"bytes,3,opt,name=error,proto3" json:"error,omitempty"`
	// message represents any relevant information associated with the step.
	Message       *wrapperspb.StringValue `protobuf:"bytes,4,opt,name=message,proto3" json:"message,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StepInfo) Reset() {
	*x = StepInfo{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StepInfo) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StepInfo) ProtoMessage() {}

func (x *StepInfo) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StepInfo.ProtoReflect.Descriptor instead.
func (*StepInfo) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{9}
}

func (x *StepInfo) GetName() *wrapperspb.StringValue {
	if x != nil {
		return x.Name
	}
	return nil
}

func (x *StepInfo) GetStatus() StepStatus {
	if x != nil {
		return x.Status
	}
	return StepStatus_STEP_STATUS_UNSPECIFIED
}

func (x *StepInfo) GetError() *wrapperspb.StringValue {
	if x != nil {
		return x.Error
	}
	return nil
}

func (x *StepInfo) GetMessage() *wrapperspb.StringValue {
	if x != nil {
		return x.Message
	}
	return nil
}

// ActionSteps tracks the status of each step within an action.
type ActionSteps struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// values maps each action step's execution sequence to its status information,
	// represented by StepInfo.
	Values        map[string]*StepInfo `protobuf:"bytes,1,rep,name=values,proto3" json:"values,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ActionSteps) Reset() {
	*x = ActionSteps{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ActionSteps) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ActionSteps) ProtoMessage() {}

func (x *ActionSteps) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ActionSteps.ProtoReflect.Descriptor instead.
func (*ActionSteps) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{10}
}

func (x *ActionSteps) GetValues() map[string]*StepInfo {
	if x != nil {
		return x.Values
	}
	return nil
}

// Stage holds the configuration and status of a stage.
type Stage struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// name is the name of the stage.
	Name *wrapperspb.StringValue `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// action is the action to perform during the stage.
	Action *Action `protobuf:"bytes,2,opt,name=action,proto3" json:"action,omitempty"`
	// rows is a series of rows of parallel stages referenced
	// by ID. See `StageConfig.rows` for more details.
	Rows *RepeatedRepeatedString `protobuf:"bytes,3,opt,name=rows,proto3" json:"rows,omitempty"`
	// status is the execution status of the stage.
	Status StageStatus `protobuf:"varint,4,opt,name=status,proto3,enum=arista.changecontrol.v1.StageStatus" json:"status,omitempty"`
	// error is any error that occured during the execution
	// of the stage.
	Error *wrapperspb.StringValue `protobuf:"bytes,5,opt,name=error,proto3" json:"error,omitempty"`
	// start_time is the time when status change to Running.
	StartTime *timestamppb.Timestamp `protobuf:"bytes,6,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	// end_time is the time when status change to Completed.
	EndTime *timestamppb.Timestamp `protobuf:"bytes,7,opt,name=end_time,json=endTime,proto3" json:"end_time,omitempty"`
	// steps represent the granular steps and their statuses
	// associated with the action performed in the Change Control stage.
	// Each stage generally defines either an action
	// or a series of sub-stages.
	Steps         *ActionSteps `protobuf:"bytes,8,opt,name=steps,proto3" json:"steps,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Stage) Reset() {
	*x = Stage{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Stage) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Stage) ProtoMessage() {}

func (x *Stage) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Stage.ProtoReflect.Descriptor instead.
func (*Stage) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{11}
}

func (x *Stage) GetName() *wrapperspb.StringValue {
	if x != nil {
		return x.Name
	}
	return nil
}

func (x *Stage) GetAction() *Action {
	if x != nil {
		return x.Action
	}
	return nil
}

func (x *Stage) GetRows() *RepeatedRepeatedString {
	if x != nil {
		return x.Rows
	}
	return nil
}

func (x *Stage) GetStatus() StageStatus {
	if x != nil {
		return x.Status
	}
	return StageStatus_STAGE_STATUS_UNSPECIFIED
}

func (x *Stage) GetError() *wrapperspb.StringValue {
	if x != nil {
		return x.Error
	}
	return nil
}

func (x *Stage) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *Stage) GetEndTime() *timestamppb.Timestamp {
	if x != nil {
		return x.EndTime
	}
	return nil
}

func (x *Stage) GetSteps() *ActionSteps {
	if x != nil {
		return x.Steps
	}
	return nil
}

// StageMap is a map from stage ID to `Stage`. This has essentially
// the same structure as `StageConfigMap`, but with each ID mapping
// to a `Stage` instead of `StageConfig`.
type StageMap struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// values is a map from stage ID to `Stage`.
	Values        map[string]*Stage `protobuf:"bytes,1,rep,name=values,proto3" json:"values,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StageMap) Reset() {
	*x = StageMap{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StageMap) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StageMap) ProtoMessage() {}

func (x *StageMap) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StageMap.ProtoReflect.Descriptor instead.
func (*StageMap) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{12}
}

func (x *StageMap) GetValues() map[string]*Stage {
	if x != nil {
		return x.Values
	}
	return nil
}

// Change holds the configuration and status of the change of a
// Change Control.
type Change struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// name is the name of the change.
	Name *wrapperspb.StringValue `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// root_stage_id is the ID of the root stage or the stage that
	// should execute first.
	RootStageId *wrapperspb.StringValue `protobuf:"bytes,2,opt,name=root_stage_id,json=rootStageId,proto3" json:"root_stage_id,omitempty"`
	// stages holds a configuration of stages and their statuses.
	Stages *StageMap `protobuf:"bytes,3,opt,name=stages,proto3" json:"stages,omitempty"`
	// notes are any notes associated with the change.
	Notes *wrapperspb.StringValue `protobuf:"bytes,4,opt,name=notes,proto3" json:"notes,omitempty"`
	// time is the time at which the change was last updated.
	Time *timestamppb.Timestamp `protobuf:"bytes,5,opt,name=time,proto3" json:"time,omitempty"`
	// user is the user by which the change was last updated.
	User          *wrapperspb.StringValue `protobuf:"bytes,6,opt,name=user,proto3" json:"user,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Change) Reset() {
	*x = Change{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Change) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Change) ProtoMessage() {}

func (x *Change) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Change.ProtoReflect.Descriptor instead.
func (*Change) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{13}
}

func (x *Change) GetName() *wrapperspb.StringValue {
	if x != nil {
		return x.Name
	}
	return nil
}

func (x *Change) GetRootStageId() *wrapperspb.StringValue {
	if x != nil {
		return x.RootStageId
	}
	return nil
}

func (x *Change) GetStages() *StageMap {
	if x != nil {
		return x.Stages
	}
	return nil
}

func (x *Change) GetNotes() *wrapperspb.StringValue {
	if x != nil {
		return x.Notes
	}
	return nil
}

func (x *Change) GetTime() *timestamppb.Timestamp {
	if x != nil {
		return x.Time
	}
	return nil
}

func (x *Change) GetUser() *wrapperspb.StringValue {
	if x != nil {
		return x.User
	}
	return nil
}

// Flag holds the configuration of a boolean flag plus some
// information about when and by whom it was set.
type Flag struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// value is the value of the flag (`true` or `false`).
	Value *wrapperspb.BoolValue `protobuf:"bytes,1,opt,name=value,proto3" json:"value,omitempty"`
	// notes are any notes associated with the setting of the flag.
	Notes *wrapperspb.StringValue `protobuf:"bytes,2,opt,name=notes,proto3" json:"notes,omitempty"`
	// time is the time at which the flag was last updated.
	Time *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=time,proto3" json:"time,omitempty"`
	// user is the user by which the flag was last updated.
	User          *wrapperspb.StringValue `protobuf:"bytes,4,opt,name=user,proto3" json:"user,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Flag) Reset() {
	*x = Flag{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Flag) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Flag) ProtoMessage() {}

func (x *Flag) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Flag.ProtoReflect.Descriptor instead.
func (*Flag) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{14}
}

func (x *Flag) GetValue() *wrapperspb.BoolValue {
	if x != nil {
		return x.Value
	}
	return nil
}

func (x *Flag) GetNotes() *wrapperspb.StringValue {
	if x != nil {
		return x.Notes
	}
	return nil
}

func (x *Flag) GetTime() *timestamppb.Timestamp {
	if x != nil {
		return x.Time
	}
	return nil
}

func (x *Flag) GetUser() *wrapperspb.StringValue {
	if x != nil {
		return x.User
	}
	return nil
}

// TimestampFlag holds the configuration of a timestamp flag
// plus some information about when and by whom it was set.
type TimestampFlag struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// value is the value of the flag (some timestamp).
	Value *timestamppb.Timestamp `protobuf:"bytes,1,opt,name=value,proto3" json:"value,omitempty"`
	// notes are any notes associated with the setting of the flag.
	Notes *wrapperspb.StringValue `protobuf:"bytes,2,opt,name=notes,proto3" json:"notes,omitempty"`
	// time is the time at which the flag was last updated.
	Time *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=time,proto3" json:"time,omitempty"`
	// user is the user by which the flag was last updated.
	User          *wrapperspb.StringValue `protobuf:"bytes,4,opt,name=user,proto3" json:"user,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TimestampFlag) Reset() {
	*x = TimestampFlag{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TimestampFlag) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TimestampFlag) ProtoMessage() {}

func (x *TimestampFlag) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TimestampFlag.ProtoReflect.Descriptor instead.
func (*TimestampFlag) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{15}
}

func (x *TimestampFlag) GetValue() *timestamppb.Timestamp {
	if x != nil {
		return x.Value
	}
	return nil
}

func (x *TimestampFlag) GetNotes() *wrapperspb.StringValue {
	if x != nil {
		return x.Notes
	}
	return nil
}

func (x *TimestampFlag) GetTime() *timestamppb.Timestamp {
	if x != nil {
		return x.Time
	}
	return nil
}

func (x *TimestampFlag) GetUser() *wrapperspb.StringValue {
	if x != nil {
		return x.User
	}
	return nil
}

// Creation holds information about when and by whom a Change Control was created.
type Creation struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// time is the time when the Change Control was created.
	Time *timestamppb.Timestamp `protobuf:"bytes,1,opt,name=time,proto3" json:"time,omitempty"`
	// user is the user who created the Change Control.
	User          *wrapperspb.StringValue `protobuf:"bytes,2,opt,name=user,proto3" json:"user,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Creation) Reset() {
	*x = Creation{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Creation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Creation) ProtoMessage() {}

func (x *Creation) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Creation.ProtoReflect.Descriptor instead.
func (*Creation) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{16}
}

func (x *Creation) GetTime() *timestamppb.Timestamp {
	if x != nil {
		return x.Time
	}
	return nil
}

func (x *Creation) GetUser() *wrapperspb.StringValue {
	if x != nil {
		return x.User
	}
	return nil
}

// Filter is used to filter changecontrols for requested device ids.
type Filter struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// device_ids includes the list of device ids to be matched with devices
	// in the changecontrol state model.
	// At least one of the provided device ids must be present in CC devices field.
	DeviceIds     *fmp.RepeatedString `protobuf:"bytes,1,opt,name=device_ids,json=deviceIds,proto3" json:"device_ids,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Filter) Reset() {
	*x = Filter{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Filter) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Filter) ProtoMessage() {}

func (x *Filter) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Filter.ProtoReflect.Descriptor instead.
func (*Filter) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{17}
}

func (x *Filter) GetDeviceIds() *fmp.RepeatedString {
	if x != nil {
		return x.DeviceIds
	}
	return nil
}

// DeviceToStageMap is a map of a device ID and its list of corresponding stages.
type DeviceToStageMap struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// values maps the device ID with the stages which are associated with a device
	// in a changecontrol state model.
	Values        map[string]*fmp.RepeatedString `protobuf:"bytes,1,rep,name=values,proto3" json:"values,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeviceToStageMap) Reset() {
	*x = DeviceToStageMap{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeviceToStageMap) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeviceToStageMap) ProtoMessage() {}

func (x *DeviceToStageMap) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeviceToStageMap.ProtoReflect.Descriptor instead.
func (*DeviceToStageMap) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{18}
}

func (x *DeviceToStageMap) GetValues() map[string]*fmp.RepeatedString {
	if x != nil {
		return x.Values
	}
	return nil
}

// ChangeControl holds the configuration and status of a Change Control.
type ChangeControl struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// key uniquely identifies the Change Control.
	Key *ChangeControlKey `protobuf:"bytes,1,opt,name=key,proto3" json:"key,omitempty"`
	// change holds the configuration and status of the change of
	// the Change Control.
	Change *Change `protobuf:"bytes,2,opt,name=change,proto3" json:"change,omitempty"`
	// approve indicates whether the Change Control was flagged
	// as approved (`approve.value` set to `true`) or unapproved
	// (`approve.value` set to `false`).
	Approve *Flag `protobuf:"bytes,3,opt,name=approve,proto3" json:"approve,omitempty"`
	// start indicates whether the Change Control was flagged to
	// start (`start.value` set to `true`) or stop (`start.value`
	// set to `false`) execution.
	Start *Flag `protobuf:"bytes,4,opt,name=start,proto3" json:"start,omitempty"`
	// status is the execution status of the Change Control.
	Status ChangeControlStatus `protobuf:"varint,5,opt,name=status,proto3,enum=arista.changecontrol.v1.ChangeControlStatus" json:"status,omitempty"`
	// error is any error that occurred during the execution of the
	// Change Control.
	Error *wrapperspb.StringValue `protobuf:"bytes,6,opt,name=error,proto3" json:"error,omitempty"`
	// schedule indicates whether the Change Control was flagged
	// to be scheduled (`schedule.value` set to some timestamp) or
	// unscheduled (`schedule.value` set to `nil`) for execution.
	Schedule *TimestampFlag `protobuf:"bytes,7,opt,name=schedule,proto3" json:"schedule,omitempty"`
	// device_ids is a list of device IDs on which the Change Control will operate.
	DeviceIds *fmp.RepeatedString `protobuf:"bytes,8,opt,name=device_ids,json=deviceIds,proto3" json:"device_ids,omitempty"`
	// device_id_to_stage_ids is a map of device IDs to the stages present in the
	// specified Change state.
	// This is not affected by the device ID based custom filtering and will contain
	// info about all the devices associated with a CC ID.
	DeviceIdToStageIds *DeviceToStageMap `protobuf:"bytes,9,opt,name=device_id_to_stage_ids,json=deviceIdToStageIds,proto3" json:"device_id_to_stage_ids,omitempty"`
	// creation holds information about when and by whom the Change Control was created.
	Creation *Creation `protobuf:"bytes,10,opt,name=creation,proto3" json:"creation,omitempty"`
	// completion_reason indicates why the Change Control completed (only relevant when
	// status is CHANGE_CONTROL_STATUS_COMPLETED).
	CompletionReason CompletionReason `protobuf:"varint,11,opt,name=completion_reason,json=completionReason,proto3,enum=arista.changecontrol.v1.CompletionReason" json:"completion_reason,omitempty"`
	// validation_statuses indicates failed validation statuses for the Change Control.
	// This is only relevant when one or more devices fail validation.
	ValidationStatuses *ChangeControlValidationStatuses `protobuf:"bytes,12,opt,name=validation_statuses,json=validationStatuses,proto3" json:"validation_statuses,omitempty"`
	// validation_detail contains additional information about the validation statuses.
	// This is only relevant when one or more devices fail validation.
	ValidationDetail *ChangeControlValidationDetail `protobuf:"bytes,13,opt,name=validation_detail,json=validationDetail,proto3" json:"validation_detail,omitempty"`
	// override_reason indicates user intent to override Change Control validation.
	OverrideReason OverrideReason `protobuf:"varint,14,opt,name=override_reason,json=overrideReason,proto3,enum=arista.changecontrol.v1.OverrideReason" json:"override_reason,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *ChangeControl) Reset() {
	*x = ChangeControl{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeControl) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeControl) ProtoMessage() {}

func (x *ChangeControl) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeControl.ProtoReflect.Descriptor instead.
func (*ChangeControl) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{19}
}

func (x *ChangeControl) GetKey() *ChangeControlKey {
	if x != nil {
		return x.Key
	}
	return nil
}

func (x *ChangeControl) GetChange() *Change {
	if x != nil {
		return x.Change
	}
	return nil
}

func (x *ChangeControl) GetApprove() *Flag {
	if x != nil {
		return x.Approve
	}
	return nil
}

func (x *ChangeControl) GetStart() *Flag {
	if x != nil {
		return x.Start
	}
	return nil
}

func (x *ChangeControl) GetStatus() ChangeControlStatus {
	if x != nil {
		return x.Status
	}
	return ChangeControlStatus_CHANGE_CONTROL_STATUS_UNSPECIFIED
}

func (x *ChangeControl) GetError() *wrapperspb.StringValue {
	if x != nil {
		return x.Error
	}
	return nil
}

func (x *ChangeControl) GetSchedule() *TimestampFlag {
	if x != nil {
		return x.Schedule
	}
	return nil
}

func (x *ChangeControl) GetDeviceIds() *fmp.RepeatedString {
	if x != nil {
		return x.DeviceIds
	}
	return nil
}

func (x *ChangeControl) GetDeviceIdToStageIds() *DeviceToStageMap {
	if x != nil {
		return x.DeviceIdToStageIds
	}
	return nil
}

func (x *ChangeControl) GetCreation() *Creation {
	if x != nil {
		return x.Creation
	}
	return nil
}

func (x *ChangeControl) GetCompletionReason() CompletionReason {
	if x != nil {
		return x.CompletionReason
	}
	return CompletionReason_COMPLETION_REASON_UNSPECIFIED
}

func (x *ChangeControl) GetValidationStatuses() *ChangeControlValidationStatuses {
	if x != nil {
		return x.ValidationStatuses
	}
	return nil
}

func (x *ChangeControl) GetValidationDetail() *ChangeControlValidationDetail {
	if x != nil {
		return x.ValidationDetail
	}
	return nil
}

func (x *ChangeControl) GetOverrideReason() OverrideReason {
	if x != nil {
		return x.OverrideReason
	}
	return OverrideReason_OVERRIDE_REASON_UNSPECIFIED
}

// ApproveConfig is used to configure the approval of a Change Control.
type ApproveConfig struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// key uniquely identifies the Change Control.
	Key *ChangeControlKey `protobuf:"bytes,1,opt,name=key,proto3" json:"key,omitempty"`
	// approve is the flag to approve (`approve.value` set to `true`)
	// or unapprove (`approve.value` set to `false`) the Change Control.
	Approve *FlagConfig `protobuf:"bytes,2,opt,name=approve,proto3" json:"approve,omitempty"`
	// version is the timestamp of the Change Control to approve.
	// This field must be set when `approve.value` is set to `true`
	// and is intended to safeguard against approving a Change Control
	// that has been updated since last read.
	Version *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=version,proto3" json:"version,omitempty"`
	// override_reason indicates user intent to override Change Control validation.
	OverrideReason OverrideReason `protobuf:"varint,4,opt,name=override_reason,json=overrideReason,proto3,enum=arista.changecontrol.v1.OverrideReason" json:"override_reason,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *ApproveConfig) Reset() {
	*x = ApproveConfig{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ApproveConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ApproveConfig) ProtoMessage() {}

func (x *ApproveConfig) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ApproveConfig.ProtoReflect.Descriptor instead.
func (*ApproveConfig) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{20}
}

func (x *ApproveConfig) GetKey() *ChangeControlKey {
	if x != nil {
		return x.Key
	}
	return nil
}

func (x *ApproveConfig) GetApprove() *FlagConfig {
	if x != nil {
		return x.Approve
	}
	return nil
}

func (x *ApproveConfig) GetVersion() *timestamppb.Timestamp {
	if x != nil {
		return x.Version
	}
	return nil
}

func (x *ApproveConfig) GetOverrideReason() OverrideReason {
	if x != nil {
		return x.OverrideReason
	}
	return OverrideReason_OVERRIDE_REASON_UNSPECIFIED
}

// ActionSummary provides detailed information about actions in a Change Control.
type ActionSummary struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// id uniquely identifies the action.
	// For built-in actions: it will be the action identifier (e.g., "setImage", "setConfig")
	// For custom scripts: it will be the script UUID
	// For task actions: it will be the task type (e.g., "ADD", "CONFIG_PUSH", "IMAGE_PUSH")
	Id *wrapperspb.StringValue `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	// name is the name of the action.
	Name *wrapperspb.StringValue `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// count is the number of times this action appears in the Change Control.
	Count         *wrapperspb.UInt32Value `protobuf:"bytes,3,opt,name=count,proto3" json:"count,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ActionSummary) Reset() {
	*x = ActionSummary{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ActionSummary) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ActionSummary) ProtoMessage() {}

func (x *ActionSummary) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ActionSummary.ProtoReflect.Descriptor instead.
func (*ActionSummary) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{21}
}

func (x *ActionSummary) GetId() *wrapperspb.StringValue {
	if x != nil {
		return x.Id
	}
	return nil
}

func (x *ActionSummary) GetName() *wrapperspb.StringValue {
	if x != nil {
		return x.Name
	}
	return nil
}

func (x *ActionSummary) GetCount() *wrapperspb.UInt32Value {
	if x != nil {
		return x.Count
	}
	return nil
}

// ActionSummaries contains a list of ActionSummary values.
type ActionSummaries struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// values is a list of ActionSummary.
	Values        []*ActionSummary `protobuf:"bytes,1,rep,name=values,proto3" json:"values,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ActionSummaries) Reset() {
	*x = ActionSummaries{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ActionSummaries) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ActionSummaries) ProtoMessage() {}

func (x *ActionSummaries) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ActionSummaries.ProtoReflect.Descriptor instead.
func (*ActionSummaries) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{22}
}

func (x *ActionSummaries) GetValues() []*ActionSummary {
	if x != nil {
		return x.Values
	}
	return nil
}

// ChangeControlSummary provides the summary of the Change Control.
type ChangeControlSummary struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// key uniquely identifies the Change Control.
	Key *ChangeControlKey `protobuf:"bytes,1,opt,name=key,proto3" json:"key,omitempty"`
	// name is the name of the Change Control.
	Name *wrapperspb.StringValue `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// status is the status of the Change Control.
	Status ChangeControlStatus `protobuf:"varint,3,opt,name=status,proto3,enum=arista.changecontrol.v1.ChangeControlStatus" json:"status,omitempty"`
	// device_count is the count of the devices impacted in the Change Control.
	DeviceCount *wrapperspb.UInt32Value `protobuf:"bytes,4,opt,name=device_count,json=deviceCount,proto3" json:"device_count,omitempty"`
	// action_summaries provides detailed information about each action type used in the Change Control.
	ActionSummaries *ActionSummaries `protobuf:"bytes,5,opt,name=action_summaries,json=actionSummaries,proto3" json:"action_summaries,omitempty"`
	// created_time is the time at which the Change Control was created.
	CreatedTime *timestamppb.Timestamp `protobuf:"bytes,6,opt,name=created_time,json=createdTime,proto3" json:"created_time,omitempty"`
	// created_user is the user by whom the Change Control was created.
	CreatedUser *wrapperspb.StringValue `protobuf:"bytes,7,opt,name=created_user,json=createdUser,proto3" json:"created_user,omitempty"`
	// last_edited_time is the time at which the Change Control was last edited.
	LastEditedTime *timestamppb.Timestamp `protobuf:"bytes,8,opt,name=last_edited_time,json=lastEditedTime,proto3" json:"last_edited_time,omitempty"`
	// last_edited_user is the user by whom the Change Control was last edited.
	LastEditedUser *wrapperspb.StringValue `protobuf:"bytes,9,opt,name=last_edited_user,json=lastEditedUser,proto3" json:"last_edited_user,omitempty"`
	// last_approved_time is the time of the last approve or unapprove operation.
	LastApprovedTime *timestamppb.Timestamp `protobuf:"bytes,10,opt,name=last_approved_time,json=lastApprovedTime,proto3" json:"last_approved_time,omitempty"`
	// last_approved_user is the user who performed the last approve or unapprove operation.
	LastApprovedUser *wrapperspb.StringValue `protobuf:"bytes,11,opt,name=last_approved_user,json=lastApprovedUser,proto3" json:"last_approved_user,omitempty"`
	// last_scheduled_time is the time of the last schedule or unschedule operation.
	LastScheduledTime *timestamppb.Timestamp `protobuf:"bytes,12,opt,name=last_scheduled_time,json=lastScheduledTime,proto3" json:"last_scheduled_time,omitempty"`
	// last_scheduled_user is the user who performed the last schedule or unschedule operation.
	LastScheduledUser *wrapperspb.StringValue `protobuf:"bytes,13,opt,name=last_scheduled_user,json=lastScheduledUser,proto3" json:"last_scheduled_user,omitempty"`
	// start_time is the time at which the Change Control execution was started.
	StartTime *timestamppb.Timestamp `protobuf:"bytes,14,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	// start_user is the user who started the Change Control execution.
	StartUser *wrapperspb.StringValue `protobuf:"bytes,15,opt,name=start_user,json=startUser,proto3" json:"start_user,omitempty"`
	// end_time is the time at which the Change Control execution was completed.
	EndTime *timestamppb.Timestamp `protobuf:"bytes,16,opt,name=end_time,json=endTime,proto3" json:"end_time,omitempty"`
	// error is any error that occurred during the execution of the Change Control.
	Error *wrapperspb.StringValue `protobuf:"bytes,17,opt,name=error,proto3" json:"error,omitempty"`
	// approved indicates whether the Change Control is currently approved or unapproved.
	Approved *wrapperspb.BoolValue `protobuf:"bytes,18,opt,name=approved,proto3" json:"approved,omitempty"`
	// completion_reason indicates why the Change Control completed (only relevant when
	// status is CHANGE_CONTROL_STATUS_COMPLETED).
	CompletionReason CompletionReason `protobuf:"varint,19,opt,name=completion_reason,json=completionReason,proto3,enum=arista.changecontrol.v1.CompletionReason" json:"completion_reason,omitempty"`
	// validation_statuses indicates the active failed validation statuses surfaced
	// in the Change Control summary. This field may contain multiple values when
	// more than one validation condition fails. For example, a Change Control may
	// be obsolete due to ordering and also temporarily blocked by concurrent
	// execution. An empty value means there are no active validation failures to
	// surface in the summary.
	ValidationStatuses *ChangeControlValidationStatuses `protobuf:"bytes,20,opt,name=validation_statuses,json=validationStatuses,proto3" json:"validation_statuses,omitempty"`
	unknownFields      protoimpl.UnknownFields
	sizeCache          protoimpl.SizeCache
}

func (x *ChangeControlSummary) Reset() {
	*x = ChangeControlSummary{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeControlSummary) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeControlSummary) ProtoMessage() {}

func (x *ChangeControlSummary) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeControlSummary.ProtoReflect.Descriptor instead.
func (*ChangeControlSummary) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{23}
}

func (x *ChangeControlSummary) GetKey() *ChangeControlKey {
	if x != nil {
		return x.Key
	}
	return nil
}

func (x *ChangeControlSummary) GetName() *wrapperspb.StringValue {
	if x != nil {
		return x.Name
	}
	return nil
}

func (x *ChangeControlSummary) GetStatus() ChangeControlStatus {
	if x != nil {
		return x.Status
	}
	return ChangeControlStatus_CHANGE_CONTROL_STATUS_UNSPECIFIED
}

func (x *ChangeControlSummary) GetDeviceCount() *wrapperspb.UInt32Value {
	if x != nil {
		return x.DeviceCount
	}
	return nil
}

func (x *ChangeControlSummary) GetActionSummaries() *ActionSummaries {
	if x != nil {
		return x.ActionSummaries
	}
	return nil
}

func (x *ChangeControlSummary) GetCreatedTime() *timestamppb.Timestamp {
	if x != nil {
		return x.CreatedTime
	}
	return nil
}

func (x *ChangeControlSummary) GetCreatedUser() *wrapperspb.StringValue {
	if x != nil {
		return x.CreatedUser
	}
	return nil
}

func (x *ChangeControlSummary) GetLastEditedTime() *timestamppb.Timestamp {
	if x != nil {
		return x.LastEditedTime
	}
	return nil
}

func (x *ChangeControlSummary) GetLastEditedUser() *wrapperspb.StringValue {
	if x != nil {
		return x.LastEditedUser
	}
	return nil
}

func (x *ChangeControlSummary) GetLastApprovedTime() *timestamppb.Timestamp {
	if x != nil {
		return x.LastApprovedTime
	}
	return nil
}

func (x *ChangeControlSummary) GetLastApprovedUser() *wrapperspb.StringValue {
	if x != nil {
		return x.LastApprovedUser
	}
	return nil
}

func (x *ChangeControlSummary) GetLastScheduledTime() *timestamppb.Timestamp {
	if x != nil {
		return x.LastScheduledTime
	}
	return nil
}

func (x *ChangeControlSummary) GetLastScheduledUser() *wrapperspb.StringValue {
	if x != nil {
		return x.LastScheduledUser
	}
	return nil
}

func (x *ChangeControlSummary) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *ChangeControlSummary) GetStartUser() *wrapperspb.StringValue {
	if x != nil {
		return x.StartUser
	}
	return nil
}

func (x *ChangeControlSummary) GetEndTime() *timestamppb.Timestamp {
	if x != nil {
		return x.EndTime
	}
	return nil
}

func (x *ChangeControlSummary) GetError() *wrapperspb.StringValue {
	if x != nil {
		return x.Error
	}
	return nil
}

func (x *ChangeControlSummary) GetApproved() *wrapperspb.BoolValue {
	if x != nil {
		return x.Approved
	}
	return nil
}

func (x *ChangeControlSummary) GetCompletionReason() CompletionReason {
	if x != nil {
		return x.CompletionReason
	}
	return CompletionReason_COMPLETION_REASON_UNSPECIFIED
}

func (x *ChangeControlSummary) GetValidationStatuses() *ChangeControlValidationStatuses {
	if x != nil {
		return x.ValidationStatuses
	}
	return nil
}

// ChangeControlValidationStatuses is a list of validation statuses for the Change Control.
type ChangeControlValidationStatuses struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// values is a list of validation statuses.
	Values        []ChangeControlValidationStatus `protobuf:"varint,1,rep,packed,name=values,proto3,enum=arista.changecontrol.v1.ChangeControlValidationStatus" json:"values,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeControlValidationStatuses) Reset() {
	*x = ChangeControlValidationStatuses{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeControlValidationStatuses) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeControlValidationStatuses) ProtoMessage() {}

func (x *ChangeControlValidationStatuses) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeControlValidationStatuses.ProtoReflect.Descriptor instead.
func (*ChangeControlValidationStatuses) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{24}
}

func (x *ChangeControlValidationStatuses) GetValues() []ChangeControlValidationStatus {
	if x != nil {
		return x.Values
	}
	return nil
}

// ChangeControlValidationDetail contains validation details for a Change Control.
type ChangeControlValidationDetail struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// devices is a list of devices that failed the Change Control validation.
	// The failure type is included in the details.
	Devices       *DeviceValidationDetails `protobuf:"bytes,1,opt,name=devices,proto3" json:"devices,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ChangeControlValidationDetail) Reset() {
	*x = ChangeControlValidationDetail{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ChangeControlValidationDetail) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ChangeControlValidationDetail) ProtoMessage() {}

func (x *ChangeControlValidationDetail) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ChangeControlValidationDetail.ProtoReflect.Descriptor instead.
func (*ChangeControlValidationDetail) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{25}
}

func (x *ChangeControlValidationDetail) GetDevices() *DeviceValidationDetails {
	if x != nil {
		return x.Devices
	}
	return nil
}

// DeviceValidationDetails is a list of devices that failed validation, along with details
// about each failure.
type DeviceValidationDetails struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// values is a list of devices with validation failures.
	Values        []*DeviceValidationDetail `protobuf:"bytes,1,rep,name=values,proto3" json:"values,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeviceValidationDetails) Reset() {
	*x = DeviceValidationDetails{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeviceValidationDetails) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeviceValidationDetails) ProtoMessage() {}

func (x *DeviceValidationDetails) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeviceValidationDetails.ProtoReflect.Descriptor instead.
func (*DeviceValidationDetails) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{26}
}

func (x *DeviceValidationDetails) GetValues() []*DeviceValidationDetail {
	if x != nil {
		return x.Values
	}
	return nil
}

// DeviceValidationDetail represents a device and its failure type.
type DeviceValidationDetail struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// device_id is the ID of the device.
	DeviceId *wrapperspb.StringValue `protobuf:"bytes,1,opt,name=device_id,json=deviceId,proto3" json:"device_id,omitempty"`
	// failure_type indicates the type of validation failure.
	FailureType DeviceValidationFailure `protobuf:"varint,2,opt,name=failure_type,json=failureType,proto3,enum=arista.changecontrol.v1.DeviceValidationFailure" json:"failure_type,omitempty"`
	// metadata is a string that contains additional information related to the failure.
	// This field may contain multiple values when more than one validation condition fails.
	// Current failures and their corresponding metadata are:
	// DEVICE_VALIDATION_FAILURE_CONFIG_OUT_OF_ORDER: ID of the newer Change Control that
	// caused the designed config to be out of order.
	// DEVICE_VALIDATION_FAILURE_IMAGE_OUT_OF_ORDER: ID of the newer Change Control that
	// caused the designed image to be out of order.
	// DEVICE_VALIDATION_FAILURE_CONCURRENT_EXECUTION: ID of the running Change Control.
	Metadata      *wrapperspb.StringValue `protobuf:"bytes,3,opt,name=metadata,proto3" json:"metadata,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeviceValidationDetail) Reset() {
	*x = DeviceValidationDetail{}
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeviceValidationDetail) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeviceValidationDetail) ProtoMessage() {}

func (x *DeviceValidationDetail) ProtoReflect() protoreflect.Message {
	mi := &file_arista_changecontrol_v1_changecontrol_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeviceValidationDetail.ProtoReflect.Descriptor instead.
func (*DeviceValidationDetail) Descriptor() ([]byte, []int) {
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP(), []int{27}
}

func (x *DeviceValidationDetail) GetDeviceId() *wrapperspb.StringValue {
	if x != nil {
		return x.DeviceId
	}
	return nil
}

func (x *DeviceValidationDetail) GetFailureType() DeviceValidationFailure {
	if x != nil {
		return x.FailureType
	}
	return DeviceValidationFailure_DEVICE_VALIDATION_FAILURE_UNSPECIFIED
}

func (x *DeviceValidationDetail) GetMetadata() *wrapperspb.StringValue {
	if x != nil {
		return x.Metadata
	}
	return nil
}

var File_arista_changecontrol_v1_changecontrol_proto protoreflect.FileDescriptor

const file_arista_changecontrol_v1_changecontrol_proto_rawDesc = "" +
	"\n" +
	"+arista/changecontrol.v1/changecontrol.proto\x12\x17arista.changecontrol.v1\x1a\x1fgoogle/protobuf/timestamp.proto\x1a\x1egoogle/protobuf/wrappers.proto\x1a\x14fmp/extensions.proto\x1a\x12fmp/wrappers.proto\"E\n" +
	"\x16RepeatedRepeatedString\x12+\n" +
	"\x06values\x18\x01 \x03(\v2\x13.fmp.RepeatedStringR\x06values\"F\n" +
	"\x10ChangeControlKey\x12,\n" +
	"\x02id\x18\x01 \x01(\v2\x1c.google.protobuf.StringValueR\x02id:\x04\x80\x8e\x19\x01\"\x9c\x01\n" +
	"\x06Action\x120\n" +
	"\x04name\x18\x01 \x01(\v2\x1c.google.protobuf.StringValueR\x04name\x126\n" +
	"\atimeout\x18\x02 \x01(\v2\x1c.google.protobuf.UInt32ValueR\atimeout\x12(\n" +
	"\x04args\x18\x03 \x01(\v2\x14.fmp.MapStringStringR\x04args\"\xbd\x01\n" +
	"\vStageConfig\x120\n" +
	"\x04name\x18\x01 \x01(\v2\x1c.google.protobuf.StringValueR\x04name\x127\n" +
	"\x06action\x18\x02 \x01(\v2\x1f.arista.changecontrol.v1.ActionR\x06action\x12C\n" +
	"\x04rows\x18\x03 \x01(\v2/.arista.changecontrol.v1.RepeatedRepeatedStringR\x04rows\"\xbe\x01\n" +
	"\x0eStageConfigMap\x12K\n" +
	"\x06values\x18\x01 \x03(\v23.arista.changecontrol.v1.StageConfigMap.ValuesEntryR\x06values\x1a_\n" +
	"\vValuesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12:\n" +
	"\x05value\x18\x02 \x01(\v2$.arista.changecontrol.v1.StageConfigR\x05value:\x028\x01\"\xf7\x01\n" +
	"\fChangeConfig\x120\n" +
	"\x04name\x18\x01 \x01(\v2\x1c.google.protobuf.StringValueR\x04name\x12@\n" +
	"\rroot_stage_id\x18\x02 \x01(\v2\x1c.google.protobuf.StringValueR\vrootStageId\x12?\n" +
	"\x06stages\x18\x03 \x01(\v2'.arista.changecontrol.v1.StageConfigMapR\x06stages\x122\n" +
	"\x05notes\x18\x04 \x01(\v2\x1c.google.protobuf.StringValueR\x05notes\"r\n" +
	"\n" +
	"FlagConfig\x120\n" +
	"\x05value\x18\x01 \x01(\v2\x1a.google.protobuf.BoolValueR\x05value\x122\n" +
	"\x05notes\x18\x02 \x01(\v2\x1c.google.protobuf.StringValueR\x05notes\"{\n" +
	"\x13TimestampFlagConfig\x120\n" +
	"\x05value\x18\x01 \x01(\v2\x1a.google.protobuf.TimestampR\x05value\x122\n" +
	"\x05notes\x18\x02 \x01(\v2\x1c.google.protobuf.StringValueR\x05notes\"\x9e\x02\n" +
	"\x13ChangeControlConfig\x12;\n" +
	"\x03key\x18\x01 \x01(\v2).arista.changecontrol.v1.ChangeControlKeyR\x03key\x12=\n" +
	"\x06change\x18\x02 \x01(\v2%.arista.changecontrol.v1.ChangeConfigR\x06change\x129\n" +
	"\x05start\x18\x03 \x01(\v2#.arista.changecontrol.v1.FlagConfigR\x05start\x12H\n" +
	"\bschedule\x18\x04 \x01(\v2,.arista.changecontrol.v1.TimestampFlagConfigR\bschedule:\x06\xfa\x8d\x19\x02rw\"\xe5\x01\n" +
	"\bStepInfo\x120\n" +
	"\x04name\x18\x01 \x01(\v2\x1c.google.protobuf.StringValueR\x04name\x12;\n" +
	"\x06status\x18\x02 \x01(\x0e2#.arista.changecontrol.v1.StepStatusR\x06status\x122\n" +
	"\x05error\x18\x03 \x01(\v2\x1c.google.protobuf.StringValueR\x05error\x126\n" +
	"\amessage\x18\x04 \x01(\v2\x1c.google.protobuf.StringValueR\amessage\"\xb5\x01\n" +
	"\vActionSteps\x12H\n" +
	"\x06values\x18\x01 \x03(\v20.arista.changecontrol.v1.ActionSteps.ValuesEntryR\x06values\x1a\\\n" +
	"\vValuesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x127\n" +
	"\x05value\x18\x02 \x01(\v2!.arista.changecontrol.v1.StepInfoR\x05value:\x028\x01\"\xd7\x03\n" +
	"\x05Stage\x120\n" +
	"\x04name\x18\x01 \x01(\v2\x1c.google.protobuf.StringValueR\x04name\x127\n" +
	"\x06action\x18\x02 \x01(\v2\x1f.arista.changecontrol.v1.ActionR\x06action\x12C\n" +
	"\x04rows\x18\x03 \x01(\v2/.arista.changecontrol.v1.RepeatedRepeatedStringR\x04rows\x12<\n" +
	"\x06status\x18\x04 \x01(\x0e2$.arista.changecontrol.v1.StageStatusR\x06status\x122\n" +
	"\x05error\x18\x05 \x01(\v2\x1c.google.protobuf.StringValueR\x05error\x129\n" +
	"\n" +
	"start_time\x18\x06 \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x125\n" +
	"\bend_time\x18\a \x01(\v2\x1a.google.protobuf.TimestampR\aendTime\x12:\n" +
	"\x05steps\x18\b \x01(\v2$.arista.changecontrol.v1.ActionStepsR\x05steps\"\xac\x01\n" +
	"\bStageMap\x12E\n" +
	"\x06values\x18\x01 \x03(\v2-.arista.changecontrol.v1.StageMap.ValuesEntryR\x06values\x1aY\n" +
	"\vValuesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x124\n" +
	"\x05value\x18\x02 \x01(\v2\x1e.arista.changecontrol.v1.StageR\x05value:\x028\x01\"\xcd\x02\n" +
	"\x06Change\x120\n" +
	"\x04name\x18\x01 \x01(\v2\x1c.google.protobuf.StringValueR\x04name\x12@\n" +
	"\rroot_stage_id\x18\x02 \x01(\v2\x1c.google.protobuf.StringValueR\vrootStageId\x129\n" +
	"\x06stages\x18\x03 \x01(\v2!.arista.changecontrol.v1.StageMapR\x06stages\x122\n" +
	"\x05notes\x18\x04 \x01(\v2\x1c.google.protobuf.StringValueR\x05notes\x12.\n" +
	"\x04time\x18\x05 \x01(\v2\x1a.google.protobuf.TimestampR\x04time\x120\n" +
	"\x04user\x18\x06 \x01(\v2\x1c.google.protobuf.StringValueR\x04user\"\xce\x01\n" +
	"\x04Flag\x120\n" +
	"\x05value\x18\x01 \x01(\v2\x1a.google.protobuf.BoolValueR\x05value\x122\n" +
	"\x05notes\x18\x02 \x01(\v2\x1c.google.protobuf.StringValueR\x05notes\x12.\n" +
	"\x04time\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\x04time\x120\n" +
	"\x04user\x18\x04 \x01(\v2\x1c.google.protobuf.StringValueR\x04user\"\xd7\x01\n" +
	"\rTimestampFlag\x120\n" +
	"\x05value\x18\x01 \x01(\v2\x1a.google.protobuf.TimestampR\x05value\x122\n" +
	"\x05notes\x18\x02 \x01(\v2\x1c.google.protobuf.StringValueR\x05notes\x12.\n" +
	"\x04time\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\x04time\x120\n" +
	"\x04user\x18\x04 \x01(\v2\x1c.google.protobuf.StringValueR\x04user\"l\n" +
	"\bCreation\x12.\n" +
	"\x04time\x18\x01 \x01(\v2\x1a.google.protobuf.TimestampR\x04time\x120\n" +
	"\x04user\x18\x02 \x01(\v2\x1c.google.protobuf.StringValueR\x04user\"<\n" +
	"\x06Filter\x122\n" +
	"\n" +
	"device_ids\x18\x01 \x01(\v2\x13.fmp.RepeatedStringR\tdeviceIds\"\xb1\x01\n" +
	"\x10DeviceToStageMap\x12M\n" +
	"\x06values\x18\x01 \x03(\v25.arista.changecontrol.v1.DeviceToStageMap.ValuesEntryR\x06values\x1aN\n" +
	"\vValuesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12)\n" +
	"\x05value\x18\x02 \x01(\v2\x13.fmp.RepeatedStringR\x05value:\x028\x01\"\x8f\b\n" +
	"\rChangeControl\x12;\n" +
	"\x03key\x18\x01 \x01(\v2).arista.changecontrol.v1.ChangeControlKeyR\x03key\x127\n" +
	"\x06change\x18\x02 \x01(\v2\x1f.arista.changecontrol.v1.ChangeR\x06change\x127\n" +
	"\aapprove\x18\x03 \x01(\v2\x1d.arista.changecontrol.v1.FlagR\aapprove\x123\n" +
	"\x05start\x18\x04 \x01(\v2\x1d.arista.changecontrol.v1.FlagR\x05start\x12D\n" +
	"\x06status\x18\x05 \x01(\x0e2,.arista.changecontrol.v1.ChangeControlStatusR\x06status\x122\n" +
	"\x05error\x18\x06 \x01(\v2\x1c.google.protobuf.StringValueR\x05error\x12B\n" +
	"\bschedule\x18\a \x01(\v2&.arista.changecontrol.v1.TimestampFlagR\bschedule\x122\n" +
	"\n" +
	"device_ids\x18\b \x01(\v2\x13.fmp.RepeatedStringR\tdeviceIds\x12]\n" +
	"\x16device_id_to_stage_ids\x18\t \x01(\v2).arista.changecontrol.v1.DeviceToStageMapR\x12deviceIdToStageIds\x12=\n" +
	"\bcreation\x18\n" +
	" \x01(\v2!.arista.changecontrol.v1.CreationR\bcreation\x12V\n" +
	"\x11completion_reason\x18\v \x01(\x0e2).arista.changecontrol.v1.CompletionReasonR\x10completionReason\x12i\n" +
	"\x13validation_statuses\x18\f \x01(\v28.arista.changecontrol.v1.ChangeControlValidationStatusesR\x12validationStatuses\x12c\n" +
	"\x11validation_detail\x18\r \x01(\v26.arista.changecontrol.v1.ChangeControlValidationDetailR\x10validationDetail\x12P\n" +
	"\x0foverride_reason\x18\x0e \x01(\x0e2'.arista.changecontrol.v1.OverrideReasonR\x0eoverrideReason:\x10\xfa\x8d\x19\x02ro\x8a\x8e\x19\x06Filter\"\x9b\x02\n" +
	"\rApproveConfig\x12;\n" +
	"\x03key\x18\x01 \x01(\v2).arista.changecontrol.v1.ChangeControlKeyR\x03key\x12=\n" +
	"\aapprove\x18\x02 \x01(\v2#.arista.changecontrol.v1.FlagConfigR\aapprove\x124\n" +
	"\aversion\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\aversion\x12P\n" +
	"\x0foverride_reason\x18\x04 \x01(\x0e2'.arista.changecontrol.v1.OverrideReasonR\x0eoverrideReason:\x06\xfa\x8d\x19\x02rw\"\xa3\x01\n" +
	"\rActionSummary\x12,\n" +
	"\x02id\x18\x01 \x01(\v2\x1c.google.protobuf.StringValueR\x02id\x120\n" +
	"\x04name\x18\x02 \x01(\v2\x1c.google.protobuf.StringValueR\x04name\x122\n" +
	"\x05count\x18\x03 \x01(\v2\x1c.google.protobuf.UInt32ValueR\x05count\"Q\n" +
	"\x0fActionSummaries\x12>\n" +
	"\x06values\x18\x01 \x03(\v2&.arista.changecontrol.v1.ActionSummaryR\x06values\"\x8f\v\n" +
	"\x14ChangeControlSummary\x12;\n" +
	"\x03key\x18\x01 \x01(\v2).arista.changecontrol.v1.ChangeControlKeyR\x03key\x120\n" +
	"\x04name\x18\x02 \x01(\v2\x1c.google.protobuf.StringValueR\x04name\x12D\n" +
	"\x06status\x18\x03 \x01(\x0e2,.arista.changecontrol.v1.ChangeControlStatusR\x06status\x12?\n" +
	"\fdevice_count\x18\x04 \x01(\v2\x1c.google.protobuf.UInt32ValueR\vdeviceCount\x12S\n" +
	"\x10action_summaries\x18\x05 \x01(\v2(.arista.changecontrol.v1.ActionSummariesR\x0factionSummaries\x12=\n" +
	"\fcreated_time\x18\x06 \x01(\v2\x1a.google.protobuf.TimestampR\vcreatedTime\x12?\n" +
	"\fcreated_user\x18\a \x01(\v2\x1c.google.protobuf.StringValueR\vcreatedUser\x12D\n" +
	"\x10last_edited_time\x18\b \x01(\v2\x1a.google.protobuf.TimestampR\x0elastEditedTime\x12F\n" +
	"\x10last_edited_user\x18\t \x01(\v2\x1c.google.protobuf.StringValueR\x0elastEditedUser\x12H\n" +
	"\x12last_approved_time\x18\n" +
	" \x01(\v2\x1a.google.protobuf.TimestampR\x10lastApprovedTime\x12J\n" +
	"\x12last_approved_user\x18\v \x01(\v2\x1c.google.protobuf.StringValueR\x10lastApprovedUser\x12J\n" +
	"\x13last_scheduled_time\x18\f \x01(\v2\x1a.google.protobuf.TimestampR\x11lastScheduledTime\x12L\n" +
	"\x13last_scheduled_user\x18\r \x01(\v2\x1c.google.protobuf.StringValueR\x11lastScheduledUser\x129\n" +
	"\n" +
	"start_time\x18\x0e \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x12;\n" +
	"\n" +
	"start_user\x18\x0f \x01(\v2\x1c.google.protobuf.StringValueR\tstartUser\x125\n" +
	"\bend_time\x18\x10 \x01(\v2\x1a.google.protobuf.TimestampR\aendTime\x122\n" +
	"\x05error\x18\x11 \x01(\v2\x1c.google.protobuf.StringValueR\x05error\x126\n" +
	"\bapproved\x18\x12 \x01(\v2\x1a.google.protobuf.BoolValueR\bapproved\x12V\n" +
	"\x11completion_reason\x18\x13 \x01(\x0e2).arista.changecontrol.v1.CompletionReasonR\x10completionReason\x12i\n" +
	"\x13validation_statuses\x18\x14 \x01(\v28.arista.changecontrol.v1.ChangeControlValidationStatusesR\x12validationStatuses:\x10\xfa\x8d\x19\x02ro\x8a\x8e\x19\x06Filter\"q\n" +
	"\x1fChangeControlValidationStatuses\x12N\n" +
	"\x06values\x18\x01 \x03(\x0e26.arista.changecontrol.v1.ChangeControlValidationStatusR\x06values\"k\n" +
	"\x1dChangeControlValidationDetail\x12J\n" +
	"\adevices\x18\x01 \x01(\v20.arista.changecontrol.v1.DeviceValidationDetailsR\adevices\"b\n" +
	"\x17DeviceValidationDetails\x12G\n" +
	"\x06values\x18\x01 \x03(\v2/.arista.changecontrol.v1.DeviceValidationDetailR\x06values\"\xe2\x01\n" +
	"\x16DeviceValidationDetail\x129\n" +
	"\tdevice_id\x18\x01 \x01(\v2\x1c.google.protobuf.StringValueR\bdeviceId\x12S\n" +
	"\ffailure_type\x18\x02 \x01(\x0e20.arista.changecontrol.v1.DeviceValidationFailureR\vfailureType\x128\n" +
	"\bmetadata\x18\x03 \x01(\v2\x1c.google.protobuf.StringValueR\bmetadata*\x7f\n" +
	"\vStageStatus\x12\x1c\n" +
	"\x18STAGE_STATUS_UNSPECIFIED\x10\x00\x12\x18\n" +
	"\x14STAGE_STATUS_RUNNING\x10\x01\x12\x1a\n" +
	"\x16STAGE_STATUS_COMPLETED\x10\x02\x12\x1c\n" +
	"\x18STAGE_STATUS_NOT_STARTED\x10\x03*\xaa\x01\n" +
	"\n" +
	"StepStatus\x12\x1b\n" +
	"\x17STEP_STATUS_UNSPECIFIED\x10\x00\x12\x1b\n" +
	"\x17STEP_STATUS_NOT_STARTED\x10\x01\x12\x17\n" +
	"\x13STEP_STATUS_RUNNING\x10\x02\x12\x18\n" +
	"\x14STEP_STATUS_FINISHED\x10\x03\x12\x17\n" +
	"\x13STEP_STATUS_SKIPPED\x10\x04\x12\x16\n" +
	"\x12STEP_STATUS_FAILED\x10\x05*\xd0\x01\n" +
	"\x13ChangeControlStatus\x12%\n" +
	"!CHANGE_CONTROL_STATUS_UNSPECIFIED\x10\x00\x12!\n" +
	"\x1dCHANGE_CONTROL_STATUS_RUNNING\x10\x01\x12#\n" +
	"\x1fCHANGE_CONTROL_STATUS_COMPLETED\x10\x02\x12#\n" +
	"\x1fCHANGE_CONTROL_STATUS_SCHEDULED\x10\x03\x12%\n" +
	"!CHANGE_CONTROL_STATUS_NOT_STARTED\x10\x04*\x91\x01\n" +
	"\x10CompletionReason\x12!\n" +
	"\x1dCOMPLETION_REASON_UNSPECIFIED\x10\x00\x12\x1d\n" +
	"\x19COMPLETION_REASON_SUCCESS\x10\x01\x12\x1c\n" +
	"\x18COMPLETION_REASON_FAILED\x10\x02\x12\x1d\n" +
	"\x19COMPLETION_REASON_STOPPED\x10\x03*R\n" +
	"\x0eOverrideReason\x12\x1f\n" +
	"\x1bOVERRIDE_REASON_UNSPECIFIED\x10\x00\x12\x1f\n" +
	"\x1bOVERRIDE_REASON_USER_FORCED\x10\x01*\xea\x01\n" +
	"\x1dChangeControlValidationStatus\x120\n" +
	",CHANGE_CONTROL_VALIDATION_STATUS_UNSPECIFIED\x10\x00\x12,\n" +
	"(CHANGE_CONTROL_VALIDATION_STATUS_SUCCESS\x10\x01\x12.\n" +
	"*CHANGE_CONTROL_VALIDATION_STATUS_OBSOLETED\x10\x02\x129\n" +
	"5CHANGE_CONTROL_VALIDATION_STATUS_CONCURRENT_EXECUTION\x10\x03*\xdd\x01\n" +
	"\x17DeviceValidationFailure\x12)\n" +
	"%DEVICE_VALIDATION_FAILURE_UNSPECIFIED\x10\x00\x121\n" +
	"-DEVICE_VALIDATION_FAILURE_CONFIG_OUT_OF_ORDER\x10\x01\x120\n" +
	",DEVICE_VALIDATION_FAILURE_IMAGE_OUT_OF_ORDER\x10\x02\x122\n" +
	".DEVICE_VALIDATION_FAILURE_CONCURRENT_EXECUTION\x10\x03B\x8e\x02\n" +
	"\x1bcom.arista.changecontrol.v1B\x12ChangecontrolProtoP\x01Z]github.com/ioplane/terraform-provider-cvp/internal/pb/arista/changecontrol.v1;changecontrolv1\xa2\x02\x03ACX\xaa\x02\x17Arista.Changecontrol.V1\xca\x02\x17Arista\\Changecontrol\\V1\xe2\x02#Arista\\Changecontrol\\V1\\GPBMetadata\xea\x02\x19Arista::Changecontrol::V1b\x06proto3"

var (
	file_arista_changecontrol_v1_changecontrol_proto_rawDescOnce sync.Once
	file_arista_changecontrol_v1_changecontrol_proto_rawDescData []byte
)

func file_arista_changecontrol_v1_changecontrol_proto_rawDescGZIP() []byte {
	file_arista_changecontrol_v1_changecontrol_proto_rawDescOnce.Do(func() {
		file_arista_changecontrol_v1_changecontrol_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_arista_changecontrol_v1_changecontrol_proto_rawDesc), len(file_arista_changecontrol_v1_changecontrol_proto_rawDesc)))
	})
	return file_arista_changecontrol_v1_changecontrol_proto_rawDescData
}

var file_arista_changecontrol_v1_changecontrol_proto_enumTypes = make([]protoimpl.EnumInfo, 7)
var file_arista_changecontrol_v1_changecontrol_proto_msgTypes = make([]protoimpl.MessageInfo, 32)
var file_arista_changecontrol_v1_changecontrol_proto_goTypes = []any{
	(StageStatus)(0),                        // 0: arista.changecontrol.v1.StageStatus
	(StepStatus)(0),                         // 1: arista.changecontrol.v1.StepStatus
	(ChangeControlStatus)(0),                // 2: arista.changecontrol.v1.ChangeControlStatus
	(CompletionReason)(0),                   // 3: arista.changecontrol.v1.CompletionReason
	(OverrideReason)(0),                     // 4: arista.changecontrol.v1.OverrideReason
	(ChangeControlValidationStatus)(0),      // 5: arista.changecontrol.v1.ChangeControlValidationStatus
	(DeviceValidationFailure)(0),            // 6: arista.changecontrol.v1.DeviceValidationFailure
	(*RepeatedRepeatedString)(nil),          // 7: arista.changecontrol.v1.RepeatedRepeatedString
	(*ChangeControlKey)(nil),                // 8: arista.changecontrol.v1.ChangeControlKey
	(*Action)(nil),                          // 9: arista.changecontrol.v1.Action
	(*StageConfig)(nil),                     // 10: arista.changecontrol.v1.StageConfig
	(*StageConfigMap)(nil),                  // 11: arista.changecontrol.v1.StageConfigMap
	(*ChangeConfig)(nil),                    // 12: arista.changecontrol.v1.ChangeConfig
	(*FlagConfig)(nil),                      // 13: arista.changecontrol.v1.FlagConfig
	(*TimestampFlagConfig)(nil),             // 14: arista.changecontrol.v1.TimestampFlagConfig
	(*ChangeControlConfig)(nil),             // 15: arista.changecontrol.v1.ChangeControlConfig
	(*StepInfo)(nil),                        // 16: arista.changecontrol.v1.StepInfo
	(*ActionSteps)(nil),                     // 17: arista.changecontrol.v1.ActionSteps
	(*Stage)(nil),                           // 18: arista.changecontrol.v1.Stage
	(*StageMap)(nil),                        // 19: arista.changecontrol.v1.StageMap
	(*Change)(nil),                          // 20: arista.changecontrol.v1.Change
	(*Flag)(nil),                            // 21: arista.changecontrol.v1.Flag
	(*TimestampFlag)(nil),                   // 22: arista.changecontrol.v1.TimestampFlag
	(*Creation)(nil),                        // 23: arista.changecontrol.v1.Creation
	(*Filter)(nil),                          // 24: arista.changecontrol.v1.Filter
	(*DeviceToStageMap)(nil),                // 25: arista.changecontrol.v1.DeviceToStageMap
	(*ChangeControl)(nil),                   // 26: arista.changecontrol.v1.ChangeControl
	(*ApproveConfig)(nil),                   // 27: arista.changecontrol.v1.ApproveConfig
	(*ActionSummary)(nil),                   // 28: arista.changecontrol.v1.ActionSummary
	(*ActionSummaries)(nil),                 // 29: arista.changecontrol.v1.ActionSummaries
	(*ChangeControlSummary)(nil),            // 30: arista.changecontrol.v1.ChangeControlSummary
	(*ChangeControlValidationStatuses)(nil), // 31: arista.changecontrol.v1.ChangeControlValidationStatuses
	(*ChangeControlValidationDetail)(nil),   // 32: arista.changecontrol.v1.ChangeControlValidationDetail
	(*DeviceValidationDetails)(nil),         // 33: arista.changecontrol.v1.DeviceValidationDetails
	(*DeviceValidationDetail)(nil),          // 34: arista.changecontrol.v1.DeviceValidationDetail
	nil,                                     // 35: arista.changecontrol.v1.StageConfigMap.ValuesEntry
	nil,                                     // 36: arista.changecontrol.v1.ActionSteps.ValuesEntry
	nil,                                     // 37: arista.changecontrol.v1.StageMap.ValuesEntry
	nil,                                     // 38: arista.changecontrol.v1.DeviceToStageMap.ValuesEntry
	(*fmp.RepeatedString)(nil),              // 39: fmp.RepeatedString
	(*wrapperspb.StringValue)(nil),          // 40: google.protobuf.StringValue
	(*wrapperspb.UInt32Value)(nil),          // 41: google.protobuf.UInt32Value
	(*fmp.MapStringString)(nil),             // 42: fmp.MapStringString
	(*wrapperspb.BoolValue)(nil),            // 43: google.protobuf.BoolValue
	(*timestamppb.Timestamp)(nil),           // 44: google.protobuf.Timestamp
}
var file_arista_changecontrol_v1_changecontrol_proto_depIdxs = []int32{
	39,  // 0: arista.changecontrol.v1.RepeatedRepeatedString.values:type_name -> fmp.RepeatedString
	40,  // 1: arista.changecontrol.v1.ChangeControlKey.id:type_name -> google.protobuf.StringValue
	40,  // 2: arista.changecontrol.v1.Action.name:type_name -> google.protobuf.StringValue
	41,  // 3: arista.changecontrol.v1.Action.timeout:type_name -> google.protobuf.UInt32Value
	42,  // 4: arista.changecontrol.v1.Action.args:type_name -> fmp.MapStringString
	40,  // 5: arista.changecontrol.v1.StageConfig.name:type_name -> google.protobuf.StringValue
	9,   // 6: arista.changecontrol.v1.StageConfig.action:type_name -> arista.changecontrol.v1.Action
	7,   // 7: arista.changecontrol.v1.StageConfig.rows:type_name -> arista.changecontrol.v1.RepeatedRepeatedString
	35,  // 8: arista.changecontrol.v1.StageConfigMap.values:type_name -> arista.changecontrol.v1.StageConfigMap.ValuesEntry
	40,  // 9: arista.changecontrol.v1.ChangeConfig.name:type_name -> google.protobuf.StringValue
	40,  // 10: arista.changecontrol.v1.ChangeConfig.root_stage_id:type_name -> google.protobuf.StringValue
	11,  // 11: arista.changecontrol.v1.ChangeConfig.stages:type_name -> arista.changecontrol.v1.StageConfigMap
	40,  // 12: arista.changecontrol.v1.ChangeConfig.notes:type_name -> google.protobuf.StringValue
	43,  // 13: arista.changecontrol.v1.FlagConfig.value:type_name -> google.protobuf.BoolValue
	40,  // 14: arista.changecontrol.v1.FlagConfig.notes:type_name -> google.protobuf.StringValue
	44,  // 15: arista.changecontrol.v1.TimestampFlagConfig.value:type_name -> google.protobuf.Timestamp
	40,  // 16: arista.changecontrol.v1.TimestampFlagConfig.notes:type_name -> google.protobuf.StringValue
	8,   // 17: arista.changecontrol.v1.ChangeControlConfig.key:type_name -> arista.changecontrol.v1.ChangeControlKey
	12,  // 18: arista.changecontrol.v1.ChangeControlConfig.change:type_name -> arista.changecontrol.v1.ChangeConfig
	13,  // 19: arista.changecontrol.v1.ChangeControlConfig.start:type_name -> arista.changecontrol.v1.FlagConfig
	14,  // 20: arista.changecontrol.v1.ChangeControlConfig.schedule:type_name -> arista.changecontrol.v1.TimestampFlagConfig
	40,  // 21: arista.changecontrol.v1.StepInfo.name:type_name -> google.protobuf.StringValue
	1,   // 22: arista.changecontrol.v1.StepInfo.status:type_name -> arista.changecontrol.v1.StepStatus
	40,  // 23: arista.changecontrol.v1.StepInfo.error:type_name -> google.protobuf.StringValue
	40,  // 24: arista.changecontrol.v1.StepInfo.message:type_name -> google.protobuf.StringValue
	36,  // 25: arista.changecontrol.v1.ActionSteps.values:type_name -> arista.changecontrol.v1.ActionSteps.ValuesEntry
	40,  // 26: arista.changecontrol.v1.Stage.name:type_name -> google.protobuf.StringValue
	9,   // 27: arista.changecontrol.v1.Stage.action:type_name -> arista.changecontrol.v1.Action
	7,   // 28: arista.changecontrol.v1.Stage.rows:type_name -> arista.changecontrol.v1.RepeatedRepeatedString
	0,   // 29: arista.changecontrol.v1.Stage.status:type_name -> arista.changecontrol.v1.StageStatus
	40,  // 30: arista.changecontrol.v1.Stage.error:type_name -> google.protobuf.StringValue
	44,  // 31: arista.changecontrol.v1.Stage.start_time:type_name -> google.protobuf.Timestamp
	44,  // 32: arista.changecontrol.v1.Stage.end_time:type_name -> google.protobuf.Timestamp
	17,  // 33: arista.changecontrol.v1.Stage.steps:type_name -> arista.changecontrol.v1.ActionSteps
	37,  // 34: arista.changecontrol.v1.StageMap.values:type_name -> arista.changecontrol.v1.StageMap.ValuesEntry
	40,  // 35: arista.changecontrol.v1.Change.name:type_name -> google.protobuf.StringValue
	40,  // 36: arista.changecontrol.v1.Change.root_stage_id:type_name -> google.protobuf.StringValue
	19,  // 37: arista.changecontrol.v1.Change.stages:type_name -> arista.changecontrol.v1.StageMap
	40,  // 38: arista.changecontrol.v1.Change.notes:type_name -> google.protobuf.StringValue
	44,  // 39: arista.changecontrol.v1.Change.time:type_name -> google.protobuf.Timestamp
	40,  // 40: arista.changecontrol.v1.Change.user:type_name -> google.protobuf.StringValue
	43,  // 41: arista.changecontrol.v1.Flag.value:type_name -> google.protobuf.BoolValue
	40,  // 42: arista.changecontrol.v1.Flag.notes:type_name -> google.protobuf.StringValue
	44,  // 43: arista.changecontrol.v1.Flag.time:type_name -> google.protobuf.Timestamp
	40,  // 44: arista.changecontrol.v1.Flag.user:type_name -> google.protobuf.StringValue
	44,  // 45: arista.changecontrol.v1.TimestampFlag.value:type_name -> google.protobuf.Timestamp
	40,  // 46: arista.changecontrol.v1.TimestampFlag.notes:type_name -> google.protobuf.StringValue
	44,  // 47: arista.changecontrol.v1.TimestampFlag.time:type_name -> google.protobuf.Timestamp
	40,  // 48: arista.changecontrol.v1.TimestampFlag.user:type_name -> google.protobuf.StringValue
	44,  // 49: arista.changecontrol.v1.Creation.time:type_name -> google.protobuf.Timestamp
	40,  // 50: arista.changecontrol.v1.Creation.user:type_name -> google.protobuf.StringValue
	39,  // 51: arista.changecontrol.v1.Filter.device_ids:type_name -> fmp.RepeatedString
	38,  // 52: arista.changecontrol.v1.DeviceToStageMap.values:type_name -> arista.changecontrol.v1.DeviceToStageMap.ValuesEntry
	8,   // 53: arista.changecontrol.v1.ChangeControl.key:type_name -> arista.changecontrol.v1.ChangeControlKey
	20,  // 54: arista.changecontrol.v1.ChangeControl.change:type_name -> arista.changecontrol.v1.Change
	21,  // 55: arista.changecontrol.v1.ChangeControl.approve:type_name -> arista.changecontrol.v1.Flag
	21,  // 56: arista.changecontrol.v1.ChangeControl.start:type_name -> arista.changecontrol.v1.Flag
	2,   // 57: arista.changecontrol.v1.ChangeControl.status:type_name -> arista.changecontrol.v1.ChangeControlStatus
	40,  // 58: arista.changecontrol.v1.ChangeControl.error:type_name -> google.protobuf.StringValue
	22,  // 59: arista.changecontrol.v1.ChangeControl.schedule:type_name -> arista.changecontrol.v1.TimestampFlag
	39,  // 60: arista.changecontrol.v1.ChangeControl.device_ids:type_name -> fmp.RepeatedString
	25,  // 61: arista.changecontrol.v1.ChangeControl.device_id_to_stage_ids:type_name -> arista.changecontrol.v1.DeviceToStageMap
	23,  // 62: arista.changecontrol.v1.ChangeControl.creation:type_name -> arista.changecontrol.v1.Creation
	3,   // 63: arista.changecontrol.v1.ChangeControl.completion_reason:type_name -> arista.changecontrol.v1.CompletionReason
	31,  // 64: arista.changecontrol.v1.ChangeControl.validation_statuses:type_name -> arista.changecontrol.v1.ChangeControlValidationStatuses
	32,  // 65: arista.changecontrol.v1.ChangeControl.validation_detail:type_name -> arista.changecontrol.v1.ChangeControlValidationDetail
	4,   // 66: arista.changecontrol.v1.ChangeControl.override_reason:type_name -> arista.changecontrol.v1.OverrideReason
	8,   // 67: arista.changecontrol.v1.ApproveConfig.key:type_name -> arista.changecontrol.v1.ChangeControlKey
	13,  // 68: arista.changecontrol.v1.ApproveConfig.approve:type_name -> arista.changecontrol.v1.FlagConfig
	44,  // 69: arista.changecontrol.v1.ApproveConfig.version:type_name -> google.protobuf.Timestamp
	4,   // 70: arista.changecontrol.v1.ApproveConfig.override_reason:type_name -> arista.changecontrol.v1.OverrideReason
	40,  // 71: arista.changecontrol.v1.ActionSummary.id:type_name -> google.protobuf.StringValue
	40,  // 72: arista.changecontrol.v1.ActionSummary.name:type_name -> google.protobuf.StringValue
	41,  // 73: arista.changecontrol.v1.ActionSummary.count:type_name -> google.protobuf.UInt32Value
	28,  // 74: arista.changecontrol.v1.ActionSummaries.values:type_name -> arista.changecontrol.v1.ActionSummary
	8,   // 75: arista.changecontrol.v1.ChangeControlSummary.key:type_name -> arista.changecontrol.v1.ChangeControlKey
	40,  // 76: arista.changecontrol.v1.ChangeControlSummary.name:type_name -> google.protobuf.StringValue
	2,   // 77: arista.changecontrol.v1.ChangeControlSummary.status:type_name -> arista.changecontrol.v1.ChangeControlStatus
	41,  // 78: arista.changecontrol.v1.ChangeControlSummary.device_count:type_name -> google.protobuf.UInt32Value
	29,  // 79: arista.changecontrol.v1.ChangeControlSummary.action_summaries:type_name -> arista.changecontrol.v1.ActionSummaries
	44,  // 80: arista.changecontrol.v1.ChangeControlSummary.created_time:type_name -> google.protobuf.Timestamp
	40,  // 81: arista.changecontrol.v1.ChangeControlSummary.created_user:type_name -> google.protobuf.StringValue
	44,  // 82: arista.changecontrol.v1.ChangeControlSummary.last_edited_time:type_name -> google.protobuf.Timestamp
	40,  // 83: arista.changecontrol.v1.ChangeControlSummary.last_edited_user:type_name -> google.protobuf.StringValue
	44,  // 84: arista.changecontrol.v1.ChangeControlSummary.last_approved_time:type_name -> google.protobuf.Timestamp
	40,  // 85: arista.changecontrol.v1.ChangeControlSummary.last_approved_user:type_name -> google.protobuf.StringValue
	44,  // 86: arista.changecontrol.v1.ChangeControlSummary.last_scheduled_time:type_name -> google.protobuf.Timestamp
	40,  // 87: arista.changecontrol.v1.ChangeControlSummary.last_scheduled_user:type_name -> google.protobuf.StringValue
	44,  // 88: arista.changecontrol.v1.ChangeControlSummary.start_time:type_name -> google.protobuf.Timestamp
	40,  // 89: arista.changecontrol.v1.ChangeControlSummary.start_user:type_name -> google.protobuf.StringValue
	44,  // 90: arista.changecontrol.v1.ChangeControlSummary.end_time:type_name -> google.protobuf.Timestamp
	40,  // 91: arista.changecontrol.v1.ChangeControlSummary.error:type_name -> google.protobuf.StringValue
	43,  // 92: arista.changecontrol.v1.ChangeControlSummary.approved:type_name -> google.protobuf.BoolValue
	3,   // 93: arista.changecontrol.v1.ChangeControlSummary.completion_reason:type_name -> arista.changecontrol.v1.CompletionReason
	31,  // 94: arista.changecontrol.v1.ChangeControlSummary.validation_statuses:type_name -> arista.changecontrol.v1.ChangeControlValidationStatuses
	5,   // 95: arista.changecontrol.v1.ChangeControlValidationStatuses.values:type_name -> arista.changecontrol.v1.ChangeControlValidationStatus
	33,  // 96: arista.changecontrol.v1.ChangeControlValidationDetail.devices:type_name -> arista.changecontrol.v1.DeviceValidationDetails
	34,  // 97: arista.changecontrol.v1.DeviceValidationDetails.values:type_name -> arista.changecontrol.v1.DeviceValidationDetail
	40,  // 98: arista.changecontrol.v1.DeviceValidationDetail.device_id:type_name -> google.protobuf.StringValue
	6,   // 99: arista.changecontrol.v1.DeviceValidationDetail.failure_type:type_name -> arista.changecontrol.v1.DeviceValidationFailure
	40,  // 100: arista.changecontrol.v1.DeviceValidationDetail.metadata:type_name -> google.protobuf.StringValue
	10,  // 101: arista.changecontrol.v1.StageConfigMap.ValuesEntry.value:type_name -> arista.changecontrol.v1.StageConfig
	16,  // 102: arista.changecontrol.v1.ActionSteps.ValuesEntry.value:type_name -> arista.changecontrol.v1.StepInfo
	18,  // 103: arista.changecontrol.v1.StageMap.ValuesEntry.value:type_name -> arista.changecontrol.v1.Stage
	39,  // 104: arista.changecontrol.v1.DeviceToStageMap.ValuesEntry.value:type_name -> fmp.RepeatedString
	105, // [105:105] is the sub-list for method output_type
	105, // [105:105] is the sub-list for method input_type
	105, // [105:105] is the sub-list for extension type_name
	105, // [105:105] is the sub-list for extension extendee
	0,   // [0:105] is the sub-list for field type_name
}

func init() { file_arista_changecontrol_v1_changecontrol_proto_init() }
func file_arista_changecontrol_v1_changecontrol_proto_init() {
	if File_arista_changecontrol_v1_changecontrol_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_arista_changecontrol_v1_changecontrol_proto_rawDesc), len(file_arista_changecontrol_v1_changecontrol_proto_rawDesc)),
			NumEnums:      7,
			NumMessages:   32,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_arista_changecontrol_v1_changecontrol_proto_goTypes,
		DependencyIndexes: file_arista_changecontrol_v1_changecontrol_proto_depIdxs,
		EnumInfos:         file_arista_changecontrol_v1_changecontrol_proto_enumTypes,
		MessageInfos:      file_arista_changecontrol_v1_changecontrol_proto_msgTypes,
	}.Build()
	File_arista_changecontrol_v1_changecontrol_proto = out.File
	file_arista_changecontrol_v1_changecontrol_proto_goTypes = nil
	file_arista_changecontrol_v1_changecontrol_proto_depIdxs = nil
}
